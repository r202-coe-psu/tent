...
*   **กาน (AIE ปี 4 — มีประสบการณ์ทำโปรเจกต์ แต่เป็น Stack ใหม่):**
    ...
    *   **หน้าที่หลัก:** พัฒนา Database Query, API Endpoints, เขียน Controller เชื่อมโยง PouchDB/CouchDB และเขียน Logic การคำนวณฝั่ง Server (เช่น การจอง, การตัดโควตา, การจับคู่ทักษะ)
...
*   **การจัดสรรงานและการแตกงานย่อย:**
    *   **[ผม] UI เจ้าหน้าที่ (Offline-first) (1.0 MD) | Priority: Medium | Due: 12/07/2026:**
        *   [ ] **พัฒนาหน้าจอสำหรับเจ้าหน้าที่คลัง รองรับการอ่านและค้นหาจาก PouchDB**
            *   *วิธีทำเบื้องต้น:* สร้าง UI สำหรับเจ้าหน้าที่คลังที่ดึงข้อมูลรายชื่อใบแจ้งบริจาคล่วงหน้ามาแสดงผลจากฐานข้อมูล local PouchDB เพื่อให้ระบบยังทำงานได้แม้สัญญานอินเทอร์เน็ตขาดหาย
        *   [ ] **หน้าจอสำหรับกรอกจำนวนที่รับจริง แล้วกด Save (`db.put`) ลง PouchDB โดยตรง**
            *   *วิธีทำเบื้องต้น:* ทำช่องบันทึกจำนวนของที่ได้รับจริงจากผู้บริจาค (อาจเท่ากับ น้อยกว่า หรือมากกว่าที่แจ้งไว้) เมื่อตรวจสอบเรียบร้อยแล้วกด Save เพื่อบันทึกลงฐานข้อมูล local PouchDB โดยตรง
    *   **[ผม] PouchDB Integration & Sync logic (1.0 MD) | Priority: High | Due: 08/07/2026:**
        *   [ ] **ยกเลิก REST API: เปลี่ยนมาเขียนโค้ดบันทึก `stock_ledger` ลง PouchDB ด้วยตัวเอง**
            *   *วิธีทำเบื้องต้น:* บันทึกข้อมูลคลังลง PouchDB ตาม schema `stock_ledger` ของทีม C (เช่น item_code, quantity, shelter_code, type: 'in') เพื่อที่ข้อมูลจะซิงค์ไปที่ศูนย์กลางและอัปเดตยอดคงเหลือคลังจริงโดยอัตโนมัติ
        *   [ ] **อัปเดตสถานะเอกสาร `donation` ลง PouchDB จาก `declared` เป็น `received`**
            *   *วิธีทำเบื้องต้น:* เมื่อทำรายการรับของเสร็จสิ้น ให้ค้นหาเอกสาร `donation` นั้นใน local PouchDB แล้วแก้ไขฟิลด์ `status` เป็น `'received'` พร้อมแนบยอดของที่ได้รับจริงและ timestamp การรับของ
    *   **[ผม] Security & Data Integrity in CouchDB (1.0 MD) | Priority: High | Due: 15/07/2026:**
        ...
        *   [ ] **สร้างระบบบันทึก `audit/movement` ลง PouchDB แนบชื่อผู้รับของ**
            *   *วิธีทำเบื้องต้น:* ทุกครั้งที่มีการตรวจสอบและรับของจริง ให้สร้างเอกสาร Audit Trail เล็ก ๆ บันทึกว่าใครเป็นผู้รับ เวลาใด และรับรายการอะไรบ้าง เพื่อใช้เป็นประวัติตรวจสอบย้อนหลัง
        *   [ ] **เขียน Automated Test ทบทวน Offline-first flow ร่วมกับ Local PouchDB**
            *   *วิธีทำเบื้องต้น:* เขียนชุดทดสอบจำลองเหตุการณ์รับของในขณะออฟไลน์ บันทึกข้อมูลลง PouchDB และจำลองการซิงค์ข้อมูลกับ CouchDB เพื่อให้มั่นใจว่าการซิงค์ไม่มีการขัดแย้งของข้อมูล (conflict)
...
*   พิมพ์ `/schedule` เมื่อต้องการตั้งนาฬิกาเตือนการทำงาน หรือการตั้งตารางตรวจเช็คสถานะการ Sync ข้อมูลของ CouchDB เป็นระยะๆ
