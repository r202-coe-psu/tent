	...
	<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-10">
		...
		<div class="rounded-2xl border-2 border-slate-200 bg-white p-8 shadow-xs">
			<div class="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
				<div class="space-y-2">
					<div class="inline-flex items-center gap-2 rounded-full border border-blue-200 bg-blue-50 px-3 py-1 text-xs font-bold text-[#0A2647]">
					...
					<p class="max-w-2xl text-base text-slate-600">
						ระบบดีไซน์มาตรฐานสำหรับศูนย์พักพิงและงานบรรเทาภัยพิบัติ เน้นความชัดเจน สว่าง คมชัด ตัวหนังสือใหญ่ (18px) และรองรับการนำไปใช้ Prompt ต่อยอดด้วย Google AI Studio
					</p>
				</div>

				<!-- Tab Switches -->
				<div class="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-100 p-1.5 shrink-0 self-start md:self-auto">
					<button
						onclick={() => (activeTab = 'components')}
						class="flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-bold transition-all {activeTab === 'components' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-600 hover:text-slate-900'}"
					>
						<Eye class="h-4 w-4" />
					...
					<button
						...
						class="flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-bold transition-all {activeTab === 'prompt-guide' ? 'bg-[#0A2647] text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'}"
					>
		...
		{#if activeTab === 'components'}
			...
			<div class="rounded-2xl border-2 border-slate-200 bg-white p-8 shadow-xs space-y-6">
				<div>
				...
				<div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
					...
					<!-- Operational Green -->
					<div class="rounded-xl border border-emerald-200 bg-emerald-50/20 p-4 space-y-2">
					...
					<!-- Heavy Load Amber -->
					<div class="rounded-xl border border-amber-200 bg-amber-50/20 p-4 space-y-2">
					...
					<!-- Critical Red -->
					<div class="rounded-xl border border-red-200 bg-red-50/20 p-4 space-y-2">
					...
					<!-- Supply Blue -->
					<div class="rounded-xl border border-sky-200 bg-sky-50/20 p-4 space-y-2">
					...
					<!-- Slate Canvas -->
					<div class="rounded-xl border border-slate-200 p-4 space-y-2">
						<div class="h-16 w-full rounded-lg bg-[#F8FAFC] border border-slate-300 shadow-xs"></div>
						<div>
			...
			<div class="rounded-2xl border-2 border-slate-200 bg-white p-8 shadow-xs space-y-6">
				<div>
					<h2 class="text-2xl font-bold text-slate-900">📊 Status Telemetry Cards (Full 360° Framing)</h2>
					<p class="text-sm text-slate-500">การ์ดแสดงตัวเลขชี้วัดความจุและสถานะวิกฤตแบบมีเส้นขอบรอบตัว</p>
				</div>

				<div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
					<!-- Operational Card -->
					<div class="card-status-operational rounded-xl p-5 space-y-3 shadow-xs">
						<div class="flex items-center justify-between">
							<span class="text-sm font-bold text-slate-700">ศูนย์เปิดทำการ</span>
							<span class="badge-status-adequate inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-bold font-mono">
						...
						<div class="flex items-baseline gap-2">
							...
							<span class="text-sm font-bold text-slate-500 font-mono">/ 24 แห่ง</span>
						</div>
						<div class="h-2 w-full overflow-hidden rounded-full bg-slate-100">
							<div class="h-full rounded-full bg-emerald-600" style="width: 75%"></div>
						</div>
					...
					<!-- Warning Card -->
					<div class="card-status-warning rounded-xl p-5 space-y-3 shadow-xs">
						<div class="flex items-center justify-between">
							<span class="text-sm font-bold text-slate-700">อัตราครองเตียง</span>
							<span class="badge-status-warning inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-bold font-mono">
						...
						<div class="flex items-baseline gap-2">
							...
							<span class="text-sm font-bold text-slate-500 font-mono">/ 4,150 เตียง</span>
						</div>
						<div class="h-2 w-full overflow-hidden rounded-full bg-slate-100">
					...
					<!-- Critical Card -->
					<div class="card-status-critical rounded-xl p-5 space-y-3 shadow-xs">
						<div class="flex items-center justify-between">
							<span class="text-sm font-bold text-slate-700">กลุ่มเปราะบาง (Red)</span>
							<span class="badge-status-critical inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-bold font-mono">
						...
						<div class="flex items-baseline gap-2">
							...
							<span class="text-sm font-bold text-slate-500 font-mono">คน</span>
						</div>
						<div class="grid grid-cols-2 gap-2 text-xs font-semibold text-slate-700 pt-1">
							<div class="rounded bg-red-100/60 p-1.5 text-center text-red-900 border border-red-200">
								ติดเตียง: <strong>48</strong>
							</div>
							<div class="rounded bg-slate-100 p-1.5 text-center text-slate-800 border border-slate-200">
					...
					<!-- Supply Card -->
					<div class="card-status-supply rounded-xl p-5 space-y-3 shadow-xs">
						<div class="flex items-center justify-between">
							<span class="text-sm font-bold text-slate-700">สำรองอาหาร (DoC)</span>
							<span class="badge-status-overstock inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-bold font-mono">
						...
						<div class="flex items-baseline gap-2">
							...
							<span class="text-sm font-bold text-slate-500 font-mono">มื้อ/วัน</span>
						</div>
						<div class="h-2 w-full overflow-hidden rounded-full bg-slate-100">
				...
				<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
					<div class="space-y-4">
						...
						<div class="flex flex-wrap items-center gap-3">
							...
							<!-- Secondary Outline -->
							<button class="inline-flex items-center justify-center gap-2 rounded-lg border-2 border-slate-300 bg-white px-5 py-3 text-base font-semibold text-slate-800 shadow-xs transition-all hover:bg-slate-50 hover:border-slate-400">
								<span>Secondary Outline</span>
							</button>

							<!-- Destructive -->
							<button class="inline-flex items-center justify-center gap-2 rounded-lg bg-red-600 px-5 py-3 text-base font-semibold text-white shadow-xs transition-all hover:bg-red-700">
					...
					<div class="space-y-4">
						...
						<div class="relative">
							<input
								...
								class="h-12 w-full rounded-lg border-2 border-slate-300 bg-slate-50 pl-10 pr-16 text-sm font-medium text-slate-900 outline-none focus:border-[#0A2647] focus:bg-white transition-all placeholder:text-slate-400"
							/>
							<Compass class="absolute left-3.5 top-3.5 h-5 w-5 text-slate-400" />
							<span class="absolute right-3 top-3 rounded border border-slate-300 bg-slate-200/60 px-2 py-0.5 font-mono text-xs font-bold text-slate-600">
								⌘K
							</span>
						</div>
					</div>
		...
		{:else}
			...
			<div class="rounded-2xl border-2 border-[#0A2647] bg-white p-8 shadow-sm space-y-6">
				<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-200 pb-6">
					<div class="space-y-1">
						...
						<p class="text-sm text-slate-600">
							คัดลอกข้อความด้านล่างนี้ไปวางในช่อง <strong>System Instructions</strong> ของ Google AI Studio เพื่อให้โมเดลเจนหน้าจอ UI ได้ตรงตามดีไซน์เป๊ะ 100%
						</p>
					</div>

					<button
						onclick={() => copyToClipboard(AI_STUDIO_SYSTEM_PROMPT)}
						class="inline-flex items-center justify-center gap-2 rounded-lg bg-[#0A2647] px-6 py-3 text-sm font-bold text-white shadow-xs transition-all hover:bg-[#051930] active:scale-95 shrink-0"
					>
						{#if copied}
				...
				<div class="relative">
					<pre class="overflow-x-auto rounded-xl border border-slate-800 bg-[#0F172A] p-6 text-xs text-slate-200 font-mono leading-relaxed whitespace-pre-wrap max-h-[600px]">{AI_STUDIO_SYSTEM_PROMPT}</pre>
				</div>
			</div>
		{/if}
