...
<form
	method="POST"
	use:form.enhance
	class="[&_[data-slot=input]]:form-control-touch space-y-6 [&_[data-slot=form-label]]:text-base sm:[&_[data-slot=form-label]]:text-sm"
>
	<Field.FieldGroup>
			...
			<div class="flex flex-col gap-5 sm:flex-row sm:items-start sm:gap-6">
				<div class="shrink-0 space-y-2 sm:w-36">
					...
					>
						...
						{:else}
							<div
								class="flex aspect-square max-w-32 flex-col items-center justify-center sm:mx-auto"
							>
					...
					<div class="grid grid-cols-1 gap-5 sm:grid-cols-2 sm:gap-4">
						<Form.Field {form} name="first_name">
							<Form.Control>
								{#snippet children({ props })}
									<Form.Label
										>{t.personal.firstName.label}
										<span class="text-destructive">*</span></Form.Label
									>
									<Input
										{...props}
