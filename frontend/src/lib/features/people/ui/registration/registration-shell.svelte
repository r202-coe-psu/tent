<script lang="ts">
	...
	import EvacueePetAssetVehicle from '../household-flows/evacuee-pet-asset-vehicle.svelte';
				...
				(entries) => {
					applyIntersectionEntries(
						...
						entries.flatMap((entry) => {
							...
							if (!(target instanceof HTMLElement) || !target.id.startsWith('reg-section-')) {
								return [];
							}
							return [
								{
									id: target.id.replace('reg-section-', ''),
