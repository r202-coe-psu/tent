	...
	function createScrollSpy(): import('svelte/attachments').Attachment {
		return (node) => {
			...
			const syncActive = () => {
				...
				if (next && sectionIds.includes(next as FormSectionId)) {
				...
				(entries) => {
					applyIntersectionEntries(
						...
						entries.flatMap((entry) => {
							...
							if (!(target instanceof HTMLElement) || !target.id.startsWith('screen-section-')) {
								return [];
							}
							return [
								{
									id: target.id.replace('screen-section-', ''),
