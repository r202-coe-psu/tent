				...
				<div class="flex min-w-0 items-start gap-3">
					...
					<div class="min-w-0 flex-1 space-y-1.5">
						...
						<div class="flex min-w-0 flex-wrap items-center gap-1.5">
							<p class="break-words text-base font-bold text-slate-900">{displayName}</p>
							{#if statusInfo}
								<span
			...
			<section class="min-w-0 space-y-2 sm:border-l sm:border-slate-200/80 sm:pl-4">
				{#if !readonly}
					...
					<button
						...
						class="inline-flex min-h-10 w-full cursor-pointer items-center justify-center gap-1.5 rounded-xl border px-3 py-2 text-sm font-semibold transition-colors hover:brightness-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-900 focus-visible:ring-offset-2 {statusInfo?.colorClass}"
					>
						<span class="size-1.5 rounded-full {statusInfo?.dotClass}"></span>
						เปลี่ยนสถานะ
					</button>
					...
					>
						<button
							...
							class="flex min-h-10 w-full cursor-pointer items-center gap-2 px-3 py-2 text-left text-sm font-semibold text-slate-800 transition-colors hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-slate-900"
						>
