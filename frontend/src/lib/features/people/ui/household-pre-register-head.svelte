...
<div class="mx-auto w-full max-w-5xl space-y-6">
	<div class="rounded-2xl border border-border bg-card p-6 shadow-sm">
		<div class="mb-6 flex flex-row items-center justify-between border-b pb-4">
			<h3 class="text-base font-bold text-slate-800 dark:text-slate-200">ข้อมูลหัวหน้าครัวเรือน</h3>
			<button
				type="button"
				onclick={handlePullIdCard}
		...
		<div class="space-y-4">
			...
			<div class="grid grid-cols-3 gap-4">
				...
				<div class="space-y-2">
					<Label for="head-phone">เบอร์โทรศัพท์</Label>
					<Input
						id="head-phone"
			...
			<div class="grid grid-cols-2 gap-4">
				<div class="space-y-2">
					<Label for="head-birthyear">ปีเกิด (พ.ศ.)</Label>
					<Input id="head-birthyear" placeholder="เช่น 2530" bind:value={form.birthYear} />
				</div>
				<div class="space-y-2">
					<Label for="head-age">อายุ (ปี)</Label>
					<Input
						id="head-age"
						placeholder="อายุ"
						bind:value={form.age}
			...
			<!-- Emergency Contact -->
			<div class="space-y-4 rounded-xl border border-border bg-slate-50/50 p-4">
				<h4
