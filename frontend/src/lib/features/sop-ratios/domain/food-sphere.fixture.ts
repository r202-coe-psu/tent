import type { FoodSphereStandard } from './food-sphere';

export const DEFAULT_FOOD_SPHERE_STANDARDS: FoodSphereStandard[] = [
	{
		_id: 'food_sphere_standard:ALL:FOOD_ENERGY',
		type: 'food_sphere_standard',
		schema_v: 1,
		target_segment: 'ALL',
		req_group_id: 'FOOD_ENERGY',
		daily_demand: 2100,
		standard_uom: 'kcal',
		effective_date: '2026-07-16',
		status: 'active',
		source: 'SPHERE_BASELINE',
		created_at: '2026-07-16T00:00:00.000Z',
		updated_at: '2026-07-16T00:00:00.000Z',
		created_by: 'system'
	},
	{
		_id: 'food_sphere_standard:ALL:FOOD_FAT',
		type: 'food_sphere_standard',
		schema_v: 1,
		target_segment: 'ALL',
		req_group_id: 'FOOD_FAT',
		daily_demand: 40,
		standard_uom: 'gram',
		effective_date: '2026-07-16',
		status: 'active',
		source: 'SPHERE_BASELINE',
		created_at: '2026-07-16T00:00:00.000Z',
		updated_at: '2026-07-16T00:00:00.000Z',
		created_by: 'system'
	},
	{
		_id: 'food_sphere_standard:ALL:FOOD_PROTEIN',
		type: 'food_sphere_standard',
		schema_v: 1,
		target_segment: 'ALL',
		req_group_id: 'FOOD_PROTEIN',
		daily_demand: 53,
		standard_uom: 'gram',
		effective_date: '2026-07-16',
		status: 'active',
		source: 'SPHERE_BASELINE',
		created_at: '2026-07-16T00:00:00.000Z',
		updated_at: '2026-07-16T00:00:00.000Z',
		created_by: 'system'
	},
	{
		_id: 'food_sphere_standard:INFANT:FOOD_ENERGY',
		type: 'food_sphere_standard',
		schema_v: 1,
		target_segment: 'INFANT',
		req_group_id: 'FOOD_ENERGY',
		daily_demand: 550,
		standard_uom: 'kcal',
		effective_date: '2026-07-16',
		status: 'active',
		source: 'SPHERE_BASELINE',
		created_at: '2026-07-16T00:00:00.000Z',
		updated_at: '2026-07-16T00:00:00.000Z',
		created_by: 'system'
	},
	{
		_id: 'food_sphere_standard:YOUNG_CHILD:FOOD_ENERGY',
		type: 'food_sphere_standard',
		schema_v: 1,
		target_segment: 'YOUNG_CHILD',
		req_group_id: 'FOOD_ENERGY',
		daily_demand: 1250,
		standard_uom: 'kcal',
		effective_date: '2026-07-16',
		status: 'active',
		source: 'SPHERE_BASELINE',
		created_at: '2026-07-16T00:00:00.000Z',
		updated_at: '2026-07-16T00:00:00.000Z',
		created_by: 'system'
	},
	{
		_id: 'food_sphere_standard:YOUNG_CHILD:FOOD_PROTEIN',
		type: 'food_sphere_standard',
		schema_v: 1,
		target_segment: 'YOUNG_CHILD',
		req_group_id: 'FOOD_PROTEIN',
		daily_demand: 25,
		standard_uom: 'gram',
		effective_date: '2026-07-16',
		status: 'active',
		source: 'SPHERE_BASELINE',
		created_at: '2026-07-16T00:00:00.000Z',
		updated_at: '2026-07-16T00:00:00.000Z',
		created_by: 'system'
	},
	{
		_id: 'food_sphere_standard:ELDERLY:FOOD_ENERGY',
		type: 'food_sphere_standard',
		schema_v: 1,
		target_segment: 'ELDERLY',
		req_group_id: 'FOOD_ENERGY',
		daily_demand: 1900,
		standard_uom: 'kcal',
		effective_date: '2026-07-16',
		status: 'active',
		source: 'SPHERE_BASELINE',
		created_at: '2026-07-16T00:00:00.000Z',
		updated_at: '2026-07-16T00:00:00.000Z',
		created_by: 'system'
	},
	{
		_id: 'food_sphere_standard:ELDERLY:FOOD_PROTEIN',
		type: 'food_sphere_standard',
		schema_v: 1,
		target_segment: 'ELDERLY',
		req_group_id: 'FOOD_PROTEIN',
		daily_demand: 60,
		standard_uom: 'gram',
		effective_date: '2026-07-16',
		status: 'active',
		source: 'SPHERE_BASELINE',
		created_at: '2026-07-16T00:00:00.000Z',
		updated_at: '2026-07-16T00:00:00.000Z',
		created_by: 'system'
	},
	{
		_id: 'food_sphere_standard:ALL:FOOD_PROTEIN_HALAL',
		type: 'food_sphere_standard',
		schema_v: 1,
		target_segment: 'ALL',
		req_group_id: 'FOOD_PROTEIN_HALAL',
		daily_demand: 53,
		standard_uom: 'gram',
		effective_date: '2026-07-16',
		status: 'active',
		source: 'SPHERE_BASELINE',
		created_at: '2026-07-16T00:00:00.000Z',
		updated_at: '2026-07-16T00:00:00.000Z',
		created_by: 'system'
	},
	{
		_id: 'food_sphere_standard:CHILD_2_5:FOOD_PROTEIN_HALAL',
		type: 'food_sphere_standard',
		schema_v: 1,
		target_segment: 'CHILD_2_5',
		req_group_id: 'FOOD_PROTEIN_HALAL',
		daily_demand: 25,
		standard_uom: 'gram',
		effective_date: '2026-07-16',
		status: 'active',
		source: 'SPHERE_BASELINE',
		created_at: '2026-07-16T00:00:00.000Z',
		updated_at: '2026-07-16T00:00:00.000Z',
		created_by: 'system'
	},
	{
		_id: 'food_sphere_standard:PREGNANT:FOOD_PROTEIN_HALAL',
		type: 'food_sphere_standard',
		schema_v: 1,
		target_segment: 'PREGNANT',
		req_group_id: 'FOOD_PROTEIN_HALAL',
		daily_demand: 70,
		standard_uom: 'gram',
		effective_date: '2026-07-16',
		status: 'active',
		source: 'SPHERE_BASELINE',
		created_at: '2026-07-16T00:00:00.000Z',
		updated_at: '2026-07-16T00:00:00.000Z',
		created_by: 'system'
	},
	{
		_id: 'food_sphere_standard:LACTATING:FOOD_PROTEIN_HALAL',
		type: 'food_sphere_standard',
		schema_v: 1,
		target_segment: 'LACTATING',
		req_group_id: 'FOOD_PROTEIN_HALAL',
		daily_demand: 75,
		standard_uom: 'gram',
		effective_date: '2026-07-16',
		status: 'active',
		source: 'SPHERE_BASELINE',
		created_at: '2026-07-16T00:00:00.000Z',
		updated_at: '2026-07-16T00:00:00.000Z',
		created_by: 'system'
	},
	{
		_id: 'food_sphere_standard:ELDERLY:FOOD_PROTEIN_HALAL',
		type: 'food_sphere_standard',
		schema_v: 1,
		target_segment: 'ELDERLY',
		req_group_id: 'FOOD_PROTEIN_HALAL',
		daily_demand: 60,
		standard_uom: 'gram',
		effective_date: '2026-07-16',
		status: 'active',
		source: 'SPHERE_BASELINE',
		created_at: '2026-07-16T00:00:00.000Z',
		updated_at: '2026-07-16T00:00:00.000Z',
		created_by: 'system'
	},
	{
		_id: 'food_sphere_standard:ALL:DRINKING_WATER',
		type: 'food_sphere_standard',
		schema_v: 1,
		target_segment: 'ALL',
		req_group_id: 'DRINKING_WATER',
		daily_demand: 3,
		standard_uom: 'liter',
		effective_date: '2026-07-16',
		status: 'active',
		source: 'SPHERE_BASELINE',
		created_at: '2026-07-16T00:00:00.000Z',
		updated_at: '2026-07-16T00:00:00.000Z',
		created_by: 'system'
	},
	{
		_id: 'food_sphere_standard:CHILD_2_5:DRINKING_WATER',
		type: 'food_sphere_standard',
		schema_v: 1,
		target_segment: 'CHILD_2_5',
		req_group_id: 'DRINKING_WATER',
		daily_demand: 1.5,
		standard_uom: 'liter',
		effective_date: '2026-07-16',
		status: 'active',
		source: 'SPHERE_BASELINE',
		created_at: '2026-07-16T00:00:00.000Z',
		updated_at: '2026-07-16T00:00:00.000Z',
		created_by: 'system'
	},
	{
		_id: 'food_sphere_standard:PREGNANT:DRINKING_WATER',
		type: 'food_sphere_standard',
		schema_v: 1,
		target_segment: 'PREGNANT',
		req_group_id: 'DRINKING_WATER',
		daily_demand: 3.5,
		standard_uom: 'liter',
		effective_date: '2026-07-16',
		status: 'active',
		source: 'SPHERE_BASELINE',
		created_at: '2026-07-16T00:00:00.000Z',
		updated_at: '2026-07-16T00:00:00.000Z',
		created_by: 'system'
	},
	{
		_id: 'food_sphere_standard:LACTATING:DRINKING_WATER',
		type: 'food_sphere_standard',
		schema_v: 1,
		target_segment: 'LACTATING',
		req_group_id: 'DRINKING_WATER',
		daily_demand: 4,
		standard_uom: 'liter',
		effective_date: '2026-07-16',
		status: 'active',
		source: 'SPHERE_BASELINE',
		created_at: '2026-07-16T00:00:00.000Z',
		updated_at: '2026-07-16T00:00:00.000Z',
		created_by: 'system'
	},
	{
		_id: 'food_sphere_standard:ELDERLY:DRINKING_WATER',
		type: 'food_sphere_standard',
		schema_v: 1,
		target_segment: 'ELDERLY',
		req_group_id: 'DRINKING_WATER',
		daily_demand: 2.5,
		standard_uom: 'liter',
		effective_date: '2026-07-16',
		status: 'active',
		source: 'SPHERE_BASELINE',
		created_at: '2026-07-16T00:00:00.000Z',
		updated_at: '2026-07-16T00:00:00.000Z',
		created_by: 'system'
	}
];
