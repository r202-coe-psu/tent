...
export async function recordFoodDistribution(
	...
	const requestedQtyDec = parseQty(input.qty);
	if (requestedQtyDec.isNegative() || requestedQtyDec.isZero()) {
		throw new WorkflowValidationError('Distribution qty must be a positive decimal string');
	}
	...
	if (ticket.requisition_type !== 'food') {
		throw new WorkflowValidationError(
			`Ticket ${ticketId} is of type '${ticket.requisition_type}', not 'food'`
		);
	}

	const targetItem = ticket.items.find((i) => i.item_id === input.item_id);
	if (!targetItem) {
		throw new WorkflowValidationError(`Item ${input.item_id} not found on ticket ${ticketId}`);
	}

	// Capacity calculation from existing logs (AC-DST-02.2)
	const existingLogs = await logRepo.list({ ticket_id: ticketId, item_id: input.item_id });
	const validLogs = existingLogs.filter((l) => l.status !== 'voided');
	const totalDistributed = validLogs.reduce((acc, l) => addQty(acc, l.qty), '0');
	const allocated = targetItem.allocated_qty || '0';
	const inHand = subQty(allocated, totalDistributed);

	if (!qtyGte(inHand, input.qty)) {
		throw new CapacityExceededError(
			`Insufficient in-hand quantity for item ${targetItem.item_name}. In-hand: ${inHand}, Requested: ${input.qty}`
		);
	}
...
export async function recordSuppliesDistribution(
	...
	if (ticket.requisition_type !== 'supplies') {
		throw new WorkflowValidationError(
			`Ticket ${ticketId} is of type '${ticket.requisition_type}', not 'supplies'`
		);
	}
