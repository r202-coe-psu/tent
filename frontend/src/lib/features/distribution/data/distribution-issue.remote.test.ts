...
let getDocHook: ((id: string) => Promise<unknown> | unknown) | undefined;
...
const supplyCtx: AuthorContext = {
	createdBy: 'Supply Coordinator',
	shelterCode: 'SH001',
	roles: ['supply_coordinator']
};

const crossShelterRegistrationCtx: AuthorContext = {
	createdBy: 'Cross-shelter Registration Staff',
	shelterCode: 'SH001',
	roles: ['SH002:registration_staff']
};
...
describe('DistributionRemoteRepository — Phase 3B Issue Execution', () => {
	...
	describe('Security & Role Authorization Matrix', () => {
		...
		it('rejects warehouse_staff, supply_coordinator, and kitchen_staff before mutation', async () => {
			seedActiveEvacuee('evacuee:01J1');
			seedActiveBatch('distribution_batch:01JBATCH1');
		...
		it('rejects a registration role scoped only to another shelter', async () => {
			seedActiveEvacuee('evacuee:01J1');
			seedActiveBatch('distribution_batch:01JBATCH1');
			const countBefore = store.size;
				...
				repo.createIssue(
					{
						...
						item_id: 'item:water',
						qty: '1',
						idempotency_key: 'idem-cross-shelter-role'
					},
					crossShelterRegistrationCtx
				)
			).rejects.toThrow(/Unauthorized/);
	...
	describe('Issue Happy Paths & Rejections', () => {
		...
		it.each(['activating', 'closing', 'closed'] as const)(
			'8. %s batch rejects new Issue admission',
			async (status) => {
				seedActiveEvacuee('evacuee:01J1');
				seedActiveBatch('distribution_batch:01J_NON_ACTIVE', {
					status,
					...(status === 'closing'
						? {
								closing_operation_id: 'close-op',
								reconciliation: [
									{
										item_id: 'item:water',
										lot_ref: 'stock_ledger:01JLOT1',
				...
				await expect(
					repo.createIssue(
						{
							batch_id: 'distribution_batch:01J_NON_ACTIVE',
							evacuee_id: 'evacuee:01J1',
							item_id: 'item:water',
							qty: '1',
		...
		it('requires an audit note for lost/damaged repeat overrides', async () => {
			seedActiveEvacuee('evacuee:01J1');
			seedActiveBatch('distribution_batch:01JBATCH1');
				...
				repo.createIssue(
					{
						...
						item_id: 'item:blanket',
						qty: '1',
						idempotency_key: 'idem-missing-override-note',
						repeat_override_reason: 'lost'
					},
					regStaffCtx
				)
			).rejects.toThrow(/repeat_override_note is required/);
		});

		it('fails closed on a mismatched deterministic Issue gate document', async () => {
			seedActiveEvacuee('evacuee:01J1');
			seedActiveBatch('distribution_batch:01JBATCH1');
			const gateId = await makeIssueGateDocId('distribution_batch:01JBATCH1');
			store.set(gateId, {
				...createDistributionIssueGate(
					{ batch_id: 'distribution_batch:01J_OTHER', state: 'open', pending_claims: [] },
					gateId.slice('distribution_issue_gate:'.length),
					regStaffCtx
				),
				_id: gateId,
				_rev: nextRev(gateId)
				...
				repo.createIssue(
					{
						...
						item_id: 'item:water',
						qty: '1',
						idempotency_key: 'idem-bad-gate'
					},
					regStaffCtx
				)
			).rejects.toBeInstanceOf(IntegrityError);
			expect(store.get(gateId)?.batch_id).toBe('distribution_batch:01J_OTHER');
			expect(
	...
	describe('Idempotency & Quantity Normalization', () => {
		it('authorizes Issue reads and filters persisted Issues by trusted shelter context', async () => {
			seedActiveEvacuee('evacuee:01J1');
			seedActiveBatch('distribution_batch:01JBATCH1');
			const issue = await repo.createIssue(
				{
					...
					item_id: 'item:water',
					qty: '1',
					idempotency_key: 'idem-scoped-read'
				},
				regStaffCtx
			);
			expect((await repo.getIssue(issue._id, warehouseCtx))?._id).toBe(issue._id);
			await expect(repo.getIssue(issue._id, supplyCtx)).rejects.toThrow(/Unauthorized/);
			await expect(repo.getIssue(issue._id, crossShelterRegistrationCtx)).rejects.toThrow(
				/Unauthorized/
			);

			store.set(issue._id, {
				...(store.get(issue._id) as InMemoryDoc),
				shelter_code: 'SH002'
			});
			expect(await repo.getIssue(issue._id, regStaffCtx)).toBeNull();
			expect(await repo.listIssuesByBatch('distribution_batch:01JBATCH1', regStaffCtx)).toEqual([]);
		});
		...
		it('returns same issue on retry with same idempotency key and same intent', async () => {
			...
			const allIssues = await repo.listIssuesByBatch('distribution_batch:01JBATCH1', regStaffCtx);
			expect(allIssues).toHaveLength(1);
		});

		it('returns a durable replay after Batch closes and recipient becomes inactive', async () => {
			seedActiveEvacuee('evacuee:01J1');
			seedActiveBatch('distribution_batch:01JBATCH1');
			const input = {
				batch_id: 'distribution_batch:01JBATCH1',
				...
				item_id: 'item:water',
				qty: '1',
				idempotency_key: 'idem-replay-after-close'
			};
			const first = await repo.createIssue(input, regStaffCtx);
			const batch = store.get(input.batch_id)!;
			store.set(input.batch_id, { ...batch, status: 'closed', _rev: nextRev(input.batch_id) });
			const recipient = store.get(input.evacuee_id)!;
			store.set(input.evacuee_id, {
				...recipient,
				current_stay: { status: 'checked_out' },
				_rev: nextRev(input.evacuee_id)
			});

			const replay = await repo.createIssue(input, regStaffCtx);
	...
	describe('DETERMINISTIC STALE-_rev ISSUE CAPACITY CAS CONFLICT TEST', () => {
		it('recomputes committed Issue truth after reading the capacity coordination revision', async () => {
			seedActiveEvacuee('evacuee:01J1');
			seedActiveBatch('distribution_batch:01JBATCH1');
			...
			store.set(capId, emptyCapacity);
			let handoffInjected = false;
			getDocHook = (id) => {
				if (id !== capId || handoffInjected) return undefined;
				handoffInjected = true;
				const issueId = `distribution_issue:${ulid()}`;
				store.set(issueId, {
					_id: issueId,
					type: 'distribution_issue',
					batch_id: 'distribution_batch:01JBATCH1',
					evacuee_id: 'evacuee:01J_OTHER',
					item_id: 'item:water',
					qty: '6',
					unit: 'bottle',
					distributed_at: now(),
					distributed_by: 'Registration Staff',
					...
					idempotency_key: 'handoff-existing',
					shelter_code: 'SH001',
					created_by: 'Registration Staff',
					created_at: now(),
					updated_at: now(),
					schema_v: 1,
					_rev: nextRev(issueId)
				});
				return JSON.parse(JSON.stringify(emptyCapacity));
			};
				...
				repo.createIssue(
					{
						...
						item_id: 'item:water',
						qty: '6',
						idempotency_key: 'idem-handoff-race'
					},
					regStaffCtx
				)
			).rejects.toBeInstanceOf(IssueCapacityError);
			expect(handoffInjected).toBe(true);
			expect(
				Array.from(store.values()).filter(
					(doc) => doc.type === 'distribution_issue' && doc.idempotency_key !== 'handoff-existing'
				)
			).toHaveLength(0);
		});

		it('does not retry a non-conflict capacity write failure', async () => {
			seedActiveEvacuee('evacuee:01J1');
			seedActiveBatch('distribution_batch:01JBATCH1');
			const capId = await makeIssueCapacityDocId('distribution_batch:01JBATCH1', 'item:water');
			let attempts = 0;
			beforeStrictWrite = (doc) => {
				if (doc._id === capId) {
					attempts += 1;
					throw new Error('capacity network failure');
				}
			};
				...
				repo.createIssue(
					{
						...
						item_id: 'item:water',
						qty: '1',
						idempotency_key: 'idem-network-failure'
					},
					regStaffCtx
				)
			).rejects.toThrow('capacity network failure');
		...
		it('prevents over-allocation when two operations race on capacity coordination', async () => {
			...
			const issues = await repo.listIssuesByBatch('distribution_batch:01JBATCH1', regStaffCtx);
