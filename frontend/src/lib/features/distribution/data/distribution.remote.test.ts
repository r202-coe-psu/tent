...
describe('DistributionRemoteRepository (Phase 3A)', () => {
	...
	describe('PR2 CAS Retry Semantics: Typed Conflict Recovery (P1 Correctness)', () => {
		it('A. Acquire claim — typed conflict: reloads latest reservation, recomputes capacity with concurrent claim, retries with new _rev, and succeeds', async () => {
			const lot = await seedInboundLot('item:water', 50, 'bottle');
			const request = await repo.createRequest(
				{
					purpose: 'Acquire conflict test',
					active_headcount_snapshot: '1',
					buffer_percent: 10,
					items: [
						{
			...
			let simulatedConflictOnce = true;
			const concurrentOpId = '01JCONCURRENTOP000000000001';
			const writtenRevs: string[] = [];
			const pendingClaimsSnapshots: Array<{ operation_id: string; qty: string }[]> = [];

			beforeStrictWrite = async (doc) => {
				if (doc._id !== resId) return;
				if (!Array.isArray(doc.pending_claims)) return;
				if (doc.pending_claims.length === 0) return;

				writtenRevs.push(doc._rev ?? '');
				pendingClaimsSnapshots.push(JSON.parse(JSON.stringify(doc.pending_claims)));

				if (simulatedConflictOnce) {
					simulatedConflictOnce = false;
					const currentInStore = store.get(resId)!;
					const newRev = nextRev(resId);
					store.set(resId, {
						...currentInStore,
						_rev: newRev,
						pending_claims: [
							{
								operation_id: concurrentOpId,
								request_id: 'distribution_request:CONCURRENT',
								batch_id: 'distribution_batch:CONCURRENT',
								item_id: 'item:water',
								lot_ref: lot._id,
								qty: '10',
								claimed_at: '2026-08-29T10:00:00.000Z'
			...
			const batch = await repo.approveRequest(
				request._id,
				[{ item_id: 'item:water', lot_ref: lot._id, qty: '15' }],
				warehouseCtx
			);

			expect(batch.status).toBe('active');
			// Three writes to reservation doc:
			// 1. Initial acquisition attempt with stale rev
			// 2. Retried acquisition write with reloaded rev and recomputed capacity
			// 3. Post-batch activation claim release cleanup
			expect(writtenRevs.length).toBe(3);
			expect(writtenRevs[0]).not.toBe(writtenRevs[1]);
			expect(writtenRevs[1]).not.toBe(writtenRevs[2]);

			// Prove reload & recompute:
			// Attempt 0 had only our claim (15)
			expect(pendingClaimsSnapshots[0]).toHaveLength(1);
			expect(pendingClaimsSnapshots[0][0].qty).toBe('15');
			// Attempt 1 reloaded state, preserved concurrent claim (10), and appended our claim (15)
			expect(pendingClaimsSnapshots[1]).toHaveLength(2);
			expect(
				pendingClaimsSnapshots[1].some((c) => c.operation_id === concurrentOpId && c.qty === '10')
			).toBe(true);
			expect(pendingClaimsSnapshots[1].some((c) => c.qty === '15')).toBe(true);
			// Attempt 2 (claim cleanup): removed our claim, preserved concurrent claim (10)
			expect(pendingClaimsSnapshots[2]).toHaveLength(1);
			expect(pendingClaimsSnapshots[2][0].operation_id).toBe(concurrentOpId);
			expect(pendingClaimsSnapshots[2][0].qty).toBe('10');

			// After full approval flow, our claim was cleaned up during batch activation, but concurrent claim remains preserved
			const finalResDoc = store.get(resId);
			expect(finalResDoc).toBeDefined();
			const finalReservation = stockLotReservationDocSchema.parse(finalResDoc);
			expect(finalReservation.pending_claims).toEqual([
		...
		it('B. Release claim — typed conflict: reloads latest reservation, preserves concurrent claims from other operations, and retries with new _rev', async () => {
			const lot = await seedInboundLot('item:cleanup', 30, 'bar');
			const request = await repo.createRequest(
				{
					purpose: 'Release conflict test',
					active_headcount_snapshot: '1',
					buffer_percent: 10,
					items: [
						{
			...
			let cleanupConflictOnce = true;
			const concurrentOpId = '01JOTHERCLAIM0000000000001';
			const cleanupRevs: string[] = [];
			const cleanupPendingClaimsSnapshots: Array<{ operation_id: string; qty: string }[]> = [];

			beforeStrictWrite = async (doc) => {
				if (doc._id !== resId) return;

				if (!claimAcquired) {
					// First write to resId is claim acquisition
					claimAcquired = true;
					return;
				}

				// Subsequent write to resId is claim release in releaseClaimWithRetry
				if (cleanupConflictOnce) {
					cleanupConflictOnce = false;
					cleanupRevs.push(doc._rev ?? '');
					cleanupPendingClaimsSnapshots.push(JSON.parse(JSON.stringify(doc.pending_claims)));

					// Simulate concurrent claim addition in store by another transaction
					const currentInStore = store.get(resId)!;
					const newRev = nextRev(resId);
					const existingClaims = Array.isArray(currentInStore.pending_claims)
						? currentInStore.pending_claims
						: [];
					store.set(resId, {
						...currentInStore,
						_rev: newRev,
						pending_claims: [
							...existingClaims,
							{
								operation_id: concurrentOpId,
								request_id: 'distribution_request:OTHER',
								batch_id: 'distribution_batch:OTHER',
								item_id: 'item:cleanup',
								lot_ref: lot._id,
								qty: '7',
								claimed_at: '2026-08-29T10:00:00.000Z'
			...
			const batch = await repo.approveRequest(
				request._id,
				[{ item_id: 'item:cleanup', lot_ref: lot._id, qty: '5' }],
				warehouseCtx
			);

			expect(batch.status).toBe('active');
			// Proves second write used the refreshed revision
			expect(cleanupRevs.length).toBe(2);
			expect(cleanupRevs[0]).not.toBe(cleanupRevs[1]);

			// Proves reload and recompute:
			// Attempt 0 tried to write empty pending_claims []
			expect(cleanupPendingClaimsSnapshots[0]).toEqual([]);
			// Attempt 1 reloaded latest reservation, removed ONLY our claim, and preserved concurrent claim [qty: '7']
			expect(cleanupPendingClaimsSnapshots[1]).toEqual([
				expect.objectContaining({
					operation_id: concurrentOpId,
					qty: '7'
				})
			...
			// Final state in store confirms concurrent claim was preserved
			const finalResDoc = store.get(resId);
			expect(finalResDoc).toBeDefined();
			const finalReservation = stockLotReservationDocSchema.parse(finalResDoc);
			expect(finalReservation.pending_claims).toHaveLength(1);
			expect(finalReservation.pending_claims[0].operation_id).toBe(concurrentOpId);
			expect(finalReservation.pending_claims[0].qty).toBe('7');
		});

		it('C. Non-conflict acquisition failure: propagates error immediately without retry loop', async () => {
			const lot = await seedInboundLot('item:fail-acq', 20, 'piece');
			const request = await repo.createRequest(
				{
					purpose: 'Fail acquisition test',
					active_headcount_snapshot: '1',
					buffer_percent: 10,
					items: [
						{
							item_id: 'item:fail-acq',
							requested_qty: '5',
							unit: 'piece',
							distribution_type_snapshot: 'consumable',
							target_qty_snapshot: '1'
			...
			beforeStrictWrite = async (doc) => {
				if (doc._id === resId) {
					writeAttempts += 1;
					throw new Error('CouchDB Internal Server Error (500)');
				}
			};
			...
			await expect(
				repo.approveRequest(
					request._id,
					[{ item_id: 'item:fail-acq', lot_ref: lot._id, qty: '5' }],
					warehouseCtx
				)
			).rejects.toThrow('CouchDB Internal Server Error (500)');

			// MUST NOT retry: write was attempted exactly once, no CAS retry loop
			expect(writeAttempts).toBe(1);
		});

		it('D. Non-conflict cleanup failure: propagates error immediately without retry loop', async () => {
			const lot = await seedInboundLot('item:fail-cleanup', 20, 'piece');
			const request = await repo.createRequest(
				{
					purpose: 'Fail cleanup test',
					active_headcount_snapshot: '1',
					buffer_percent: 10,
					items: [
						{
							item_id: 'item:fail-cleanup',
							requested_qty: '5',
							unit: 'piece',
							distribution_type_snapshot: 'consumable',
							target_qty_snapshot: '1'
			...
			beforeStrictWrite = async (doc) => {
				if (doc._id === resId) {
					...
					// Subsequent write to resId is cleanup
					cleanupAttempts += 1;
					throw new Error('Network timeout during cleanup (504)');
				}
			};
			...
			await expect(
				repo.approveRequest(
					request._id,
					[{ item_id: 'item:fail-cleanup', lot_ref: lot._id, qty: '5' }],
					warehouseCtx
				)
			).rejects.toThrow('Network timeout during cleanup (504)');

			// MUST NOT retry: cleanup write was attempted exactly once, no CAS retry loop
			expect(cleanupAttempts).toBe(1);
		});
	});

	describe('PR2 Hardened Reservation Cleanup & Identity (P1 Hardening)', () => {
		it('A. valid cleanup: removes current claim and preserves other operation claim', async () => {
			const lot = await seedInboundLot('item:valid-clean', 20, 'piece');
			const request = await repo.createRequest(
				{
					purpose: 'Valid cleanup test',
					active_headcount_snapshot: '1',
					buffer_percent: 10,
					items: [
						{
							item_id: 'item:valid-clean',
							requested_qty: '5',
							unit: 'piece',
							distribution_type_snapshot: 'consumable',
							target_qty_snapshot: '1'
			...
			const resId = await makeLotReservationDocId(lot._id);
			const concurrentOpId = '01JCONCURRENTCLAIM000000001';
			store.set(resId, {
				...createStockLotReservation(
					{
						lot_ref: lot._id,
			...
			const batch = await repo.approveRequest(
				request._id,
				[{ item_id: 'item:valid-clean', lot_ref: lot._id, qty: '5' }],
				warehouseCtx
			);
			...
			const finalResDoc = store.get(resId);
			expect(finalResDoc).toBeDefined();
			const finalRes = stockLotReservationDocSchema.parse(finalResDoc);
			expect(finalRes.pending_claims).toEqual([
				expect.objectContaining({
					operation_id: concurrentOpId,
					qty: '3'
		...
		it('B. malformed reservation shape: cleanup rejects and no mutation write occurs', async () => {
			const lot = await seedInboundLot('item:malformed-clean', 20, 'piece');
			const request = await repo.createRequest(
				{
					purpose: 'Malformed cleanup test',
					active_headcount_snapshot: '1',
					buffer_percent: 10,
					items: [
						{
							item_id: 'item:malformed-clean',
							requested_qty: '5',
							unit: 'piece',
							distribution_type_snapshot: 'consumable',
							target_qty_snapshot: '1'
			...
			beforeStrictWrite = async (doc) => {
				if (doc._id === resId) {
					...
					cleanupWrites += 1;
				} else if (doc.type === 'distribution_request' && doc.status === 'approved') {
					// Corrupt reservation before cleanup reads it
					const cur = store.get(resId)!;
					store.set(resId, {
						...cur,
						type: 'malformed_type',
						_rev: nextRev(resId)
			...
			await expect(
				repo.approveRequest(
					request._id,
					[{ item_id: 'item:malformed-clean', lot_ref: lot._id, qty: '5' }],
					warehouseCtx
				)
			).rejects.toThrow(IntegrityError);

			// Assert no mutation write occurred on the malformed document
			expect(cleanupWrites).toBe(0);
		});

		it('C. wrong lot_ref: deterministic ID exists but contains another lot_ref -> fail closed, no cleanup PUT', async () => {
			const lot = await seedInboundLot('item:wrong-lot', 20, 'piece');
			const request = await repo.createRequest(
				{
					purpose: 'Wrong lot test',
					active_headcount_snapshot: '1',
					buffer_percent: 10,
					items: [
						{
							item_id: 'item:wrong-lot',
							requested_qty: '5',
							unit: 'piece',
							distribution_type_snapshot: 'consumable',
							target_qty_snapshot: '1'
			...
			beforeStrictWrite = async (doc) => {
				if (doc._id === resId) {
					...
					// Replace lot_ref with a different physical lot
					const cur = store.get(resId)!;
					store.set(resId, {
						...cur,
						lot_ref: 'stock_ledger:01JWRONGLOT0000000000000',
						_rev: nextRev(resId)
					});
				}
			};
			...
			await expect(
				repo.approveRequest(
					request._id,
					[{ item_id: 'item:wrong-lot', lot_ref: lot._id, qty: '5' }],
					warehouseCtx
				)
			).rejects.toThrow(/lot_ref mismatch/);

			expect(cleanupWrites).toBe(0);
		});

		it('D. wrong shelter_code: document contains another shelter -> fail closed, no cleanup PUT', async () => {
			const lot = await seedInboundLot('item:wrong-shelter', 20, 'piece');
			const request = await repo.createRequest(
				{
					purpose: 'Wrong shelter test',
					active_headcount_snapshot: '1',
					buffer_percent: 10,
					items: [
						{
							item_id: 'item:wrong-shelter',
							requested_qty: '5',
							unit: 'piece',
							distribution_type_snapshot: 'consumable',
							target_qty_snapshot: '1'
			...
			beforeStrictWrite = async (doc) => {
				if (doc._id === resId) {
					...
					// Tamper shelter_code to another shelter
					const cur = store.get(resId)!;
					store.set(resId, {
						...cur,
						shelter_code: 'SH999',
			...
			await expect(
				repo.approveRequest(
					request._id,
					[{ item_id: 'item:wrong-shelter', lot_ref: lot._id, qty: '5' }],
					warehouseCtx
				)
			).rejects.toThrow(/shelter_code mismatch/);

			expect(cleanupWrites).toBe(0);
		});

		it('E. typed ConflictError during cleanup: reloads latest reservation, parses + identity-checks again, preserves concurrent claim, and retries successfully', async () => {
			const lot = await seedInboundLot('item:conflict-clean', 20, 'piece');
			const request = await repo.createRequest(
				{
					purpose: 'Conflict clean test',
					active_headcount_snapshot: '1',
					buffer_percent: 10,
					items: [
						{
							item_id: 'item:conflict-clean',
							requested_qty: '5',
							unit: 'piece',
							distribution_type_snapshot: 'consumable',
							target_qty_snapshot: '1'
			...
			let claimAcquired = false;
			let cleanupAttempts = 0;
			const concurrentOpId = '01JCONCURRENT00000000001';
			...
			beforeStrictWrite = async (doc) => {
				if (doc._id === resId) {
					...
					if (cleanupAttempts === 1) {
						const cur = store.get(resId)!;
						const existingClaims = Array.isArray(cur.pending_claims) ? cur.pending_claims : [];
						store.set(resId, {
							...cur,
							_rev: nextRev(resId),
							pending_claims: [
			...
			const batch = await repo.approveRequest(
				request._id,
				[{ item_id: 'item:conflict-clean', lot_ref: lot._id, qty: '5' }],
				warehouseCtx
			);
		...
		it('F. corrupted document discovered after conflict reload: fails closed immediately without retrying blindly', async () => {
			const lot = await seedInboundLot('item:conflict-corrupt', 20, 'piece');
			const request = await repo.createRequest(
				{
					purpose: 'Conflict corrupt test',
					active_headcount_snapshot: '1',
					buffer_percent: 10,
					items: [
						{
							item_id: 'item:conflict-corrupt',
							requested_qty: '5',
							unit: 'piece',
							distribution_type_snapshot: 'consumable',
							target_qty_snapshot: '1'
			...
			beforeStrictWrite = async (doc) => {
				if (doc._id === resId) {
					...
					cleanupAttempts += 1;
					if (cleanupAttempts === 1) {
						// Simulate conflict caused by an attacker or corruption tampering shelter_code
						const cur = store.get(resId)!;
						store.set(resId, {
							...cur,
							shelter_code: 'SH_ATTACKER',
							_rev: nextRev(resId)
						});
						throw new ConflictError(doc._id);
					}
			...
			await expect(
				repo.approveRequest(
					request._id,
					[{ item_id: 'item:conflict-corrupt', lot_ref: lot._id, qty: '5' }],
					warehouseCtx
				)
			).rejects.toThrow(IntegrityError);

			// Must NOT continue retrying: attempt 1 discovered corrupted doc and failed closed immediately
			expect(cleanupAttempts).toBe(1);
		});
	});
});
