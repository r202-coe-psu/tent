...
/** Asserts that incoming reconciliation input for a retry matches the persisted close operation. */
export function assertSemanticClosingMatch(
	...
	const existingByLot = new Map(existing.map((row) => [row.lot_ref, row]));
	const existingTotalsByItem = new Map<
		string,
		{ damaged: string; lost: string; damaged_note?: string; lost_note?: string }
	>();
	for (const row of existing) {
		const current = existingTotalsByItem.get(row.item_id) ?? { damaged: '0', lost: '0' };
		existingTotalsByItem.set(row.item_id, {
			damaged: addQty(current.damaged, row.damaged_qty),
			lost: addQty(current.lost, row.lost_qty),
			damaged_note: row.damaged_note ?? current.damaged_note,
			lost_note: row.lost_note ?? current.lost_note
		});
	}
	...
	for (const input of inputReconciliation) {
		...
		const match = input.lot_ref
			? existingByLot.get(input.lot_ref)
			: existingTotalsByItem.get(input.item_id);
		if (!match) {
			throw new BatchClosingConflictError(
				input.lot_ref
					? `Input lot_ref ${input.lot_ref} is not present in existing closing reconciliation`
					: `Input item_id ${input.item_id} is not present in existing closing reconciliation`
			);
		}
		if ('item_id' in match && match.item_id !== input.item_id) {
			throw new BatchClosingConflictError(
				`Input item_id ${input.item_id} does not match existing closing reconciliation`
			);
		}
		const persistedDamaged = 'damaged_qty' in match ? match.damaged_qty : match.damaged;
		const persistedLost = 'lost_qty' in match ? match.lost_qty : match.lost;
		if (persistedDamaged !== inputDamaged || persistedLost !== inputLost) {
			throw new BatchClosingConflictError(
				`Input damaged/lost quantities conflict with persisted closing reconciliation`
			);
		}
		if (
			...
			throw new BatchClosingConflictError(
				`Input notes conflict with persisted closing reconciliation`
			);
		}
	}
}
