...
describe('DistributionRemoteRepository closeBatch (Phase A Step 2)', () => {
	...
	describe('Step 2.5 Correctness & Concurrency Audit', () => {
		...
		it('Case 8: closed retry behavior matches documented contract', async () => {
			...
			const returnLedgerId = `stock_ledger:${closingOperationId}:0`;
			store.set(returnLedgerId, {
				_id: returnLedgerId,
				_rev: nextRev(returnLedgerId),
				type: 'stock_ledger',
				schema_v: 4,
				shelter_code: 'SH001',
				created_by: 'ws-1',
	...
	describe('Step 3 — Retry-Safe distribution_return Stock Ledger & Crash Recovery', () => {
		...
		it('T3-20: final closing -> closed CAS retry remains idempotent', async () => {
			...
			putDocStrictHook = (doc) => {
				if (doc.type === 'distribution_batch' && doc.status === 'closed' && firstAttempt) {
					...
					throw new ConflictError('Conflict on final close');
				}
			};
		...
		it('T3-20b: final close reloads and retries when closing revision changes', async () => {
			const batch = seedBatch();
			seedIssue(batch._id, 'item:blanket', '30');
			let finalAttempts = 0;
			putDocStrictHook = (doc) => {
				if (doc.type === 'distribution_batch' && doc.status === 'closed') {
					finalAttempts += 1;
					if (finalAttempts === 1) {
						const current = store.get(doc._id)!;
						store.set(doc._id, { ...current, _rev: nextRev(doc._id) });
					}
				}
			};
		...
		it('T3-20c: final close does not retry a non-conflict write failure', async () => {
			const batch = seedBatch();
			seedIssue(batch._id, 'item:blanket', '30');
			let finalAttempts = 0;
			putDocStrictHook = (doc) => {
				if (doc.type === 'distribution_batch' && doc.status === 'closed') {
					finalAttempts += 1;
					throw new Error('final close network failure');
				}
			};

			await expect(repo.closeBatch(batch._id, {}, WAREHOUSE_CTX)).rejects.toThrow(
				'final close network failure'
			);
			expect(finalAttempts).toBe(1);
			expect(store.get(batch._id)?.status).toBe('closing');
			const operationId = store.get(batch._id)?.closing_operation_id;

			putDocStrictHook = null;
			const recovered = await repo.closeBatch(batch._id, {}, WAREHOUSE_CTX);
			expect(recovered.status).toBe('closed');
			expect(recovered.closing_operation_id).toBe(operationId);
			expect(
		...
		it('GATE-02: active Batch resumes an already-sealed close operation after uncertainty', async () => {
			const batch = seedBatch();
			seedIssue(batch._id, 'item:blanket', '39');
			const gateId = await makeIssueGateDocId(batch._id);
			const priorOperationId = ulid();
			store.set(gateId, {
				...createDistributionIssueGate(
					{
						batch_id: batch._id,
						state: 'sealed',
						pending_claims: [],
						closing_operation_id: priorOperationId
					},
			...
			const closed = await repo.closeBatch(batch._id, {}, WAREHOUSE_CTX);
			expect(closed.status).toBe('closed');
			expect(closed.closing_operation_id).toBe(priorOperationId);
			expect(closed.return_ledger_ids).toEqual([`stock_ledger:${priorOperationId}:0`]);
		});
		...
		it('GATE-08: gate reopen retries a real ConflictError and confirms the latest state', async () => {
			const batch = seedBatch();
			seedIssue(batch._id, 'item:blanket', '39');
			const gateId = await makeIssueGateDocId(batch._id);
			let reopenAttempts = 0;
			putDocStrictHook = (doc) => {
				if (doc._id === gateId && doc.state === 'open' && doc._rev) {
					reopenAttempts += 1;
					if (reopenAttempts === 1) {
						const current = requireIssueGate(gateId);
						store.set(gateId, { ...current, _rev: nextRev(gateId) });
					}
				}
			};
			...
			expect(requireIssueGate(gateId)).toMatchObject({
				state: 'open',
				pending_claims: []
			});
		});

		it('GATE-09: gate reopen does not retry or hide a non-conflict failure', async () => {
			const batch = seedBatch();
			seedIssue(batch._id, 'item:blanket', '39');
			const gateId = await makeIssueGateDocId(batch._id);
			let reopenAttempts = 0;
			putDocStrictHook = (doc) => {
				if (doc._id === gateId && doc.state === 'open' && doc._rev) {
					reopenAttempts += 1;
					throw new Error('gate network failure');
				}
			};
			...
			).rejects.toThrow(/Close gate recovery could not be confirmed.*gate network failure/);

			expect(reopenAttempts).toBe(1);
			expect(requireIssueGate(gateId).state).toBe('sealed');
		});

		it('fails closed when matching Issue history belongs to another shelter', async () => {
			const batch = seedBatch();
			const issue = seedIssue(batch._id, 'item:blanket', '1');
			store.set(issue._id, { ...issue, shelter_code: 'SH002' });

			await expect(repo.closeBatch(batch._id, {}, WAREHOUSE_CTX)).rejects.toThrow(
				/outside the batch shelter scope/
			);
