...
// ---- RequisitionTicket (schema.md §2.29, CR-121 + kitchen carve-out CR-141) ----
// Scope: `requisition_type: 'kitchen'` only. `food`/`supplies`/`transfer` are not
// implemented by this feature yet — see CR-141 for why the status enum here is a
...
export interface RequisitionTicket extends BaseDoc {
	...
	status: TicketStatus;
	meal_plan_id: string; // kitchen-only idempotency/link key (CR-141)
 ...
 * hasn't been touched by the warehouse yet (CR-142). Not self-approve/bypass —
...
/**
 * One-click approve (CR-143 — supersedes CR-141 §2.2's 3-role split for
...
/** kitchen_staff confirms receipt — IN_TRANSIT → COMPLETED (CR-141 §2.1/§2.2). */
