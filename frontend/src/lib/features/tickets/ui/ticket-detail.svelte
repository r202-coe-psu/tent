<script lang="ts">
	...
	// CR-128: kitchen tickets go PENDING_PICK → COMPLETED in one click, so the
...
<div class="mx-auto max-w-4xl space-y-5 bg-slate-50/60 p-4 sm:p-6">
	...
	{:else}
		...
		{:else if ticket.status === 'READY_FOR_DISPATCH' || ticket.status === 'IN_TRANSIT'}
			<!-- Historical tickets only (created before CR-128) — walk the old 3-role
			...
			>
				ตั๋วนี้เปิดก่อนปรับเป็นอนุมัติคลิกเดียว (CR-128) — ดำเนินการต่อตามขั้นตอนเดิม:
