<script lang="ts">
	...
	function syncOverrides() {
		...
		for (const key of SOP_RATIO_KEYS)
			if (enabledOverrides[key]) draft[key] = ratioValues[key] ?? '';
	...
	function handleModeKeydown(event: KeyboardEvent) {
		let nextMode: ScenarioMode | null = null;
		if (event.key === 'ArrowRight' || event.key === 'ArrowLeft') {
			nextMode = mode === 'general' ? 'sop_override' : 'general';
		} else if (event.key === 'Home') {
			nextMode = 'general';
		} else if (event.key === 'End') {
			nextMode = 'sop_override';
		}
		if (!nextMode) return;
		event.preventDefault();
		setMode(nextMode);
		queueMicrotask(() => document.getElementById(`scenario-mode-${nextMode}`)?.focus());
	}
...
<form method="POST" use:enhance class="scenario-form">
	<div class="scenario-mode" role="tablist" aria-label="รูปแบบการจำลอง">
		<button
			id="scenario-mode-general"
			type="button"
			role="tab"
			aria-selected={mode === 'general'}
			aria-controls="scenario-mode-panel"
			tabindex={mode === 'general' ? 0 : -1}
		...
		<button
			id="scenario-mode-sop_override"
			type="button"
			role="tab"
			aria-selected={mode === 'sop_override'}
			aria-controls="scenario-mode-panel"
			tabindex={mode === 'sop_override' ? 0 : -1}
	...
	<div
		id="scenario-mode-panel"
		class="scenario-fields"
		role="tabpanel"
		aria-labelledby={`scenario-mode-${mode}`}
	>
	...
	<div class="scenario-actions">
		...
		>
		<Button type="submit" disabled={running} class="scenario-run-button"
