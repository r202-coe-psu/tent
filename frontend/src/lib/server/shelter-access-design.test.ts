...
describe('buildValidateDocUpdate', () => {
	...
	describe('stock_ledger role gate', () => {
		...
		it('enforces distribution_return ref, lot, positive qty, and precision invariants', () => {
			const valid = ledger({
				qty: '2.5',
				reason: 'distribution_return',
				ref_id: 'distribution_batch:01J',
			...
			expect(() => compile()(valid, null, WAREHOUSE)).not.toThrow();

			// Valid decimal edge cases
			expect(() => compile()({ ...valid, qty: '1' }, null, WAREHOUSE)).not.toThrow();
			expect(() => compile()({ ...valid, qty: '0.1' }, null, WAREHOUSE)).not.toThrow();
			expect(() => compile()({ ...valid, qty: '0.0001' }, null, WAREHOUSE)).not.toThrow();
			expect(() => compile()({ ...valid, qty: '10.25' }, null, WAREHOUSE)).not.toThrow();

			// REF invariants: missing/null or invalid prefix
			expectForbidden(
				() => compile()({ ...valid, ref_id: null }, null, WAREHOUSE),
				/Distribution return stock ledger requires distribution_batch ref_id/
			);
			expectForbidden(
				() => compile()({ ...valid, ref_id: 'donation:01J' }, null, WAREHOUSE),
				/Distribution return stock ledger requires distribution_batch ref_id/
			...
			expectForbidden(
				() => compile()({ ...valid, ref_id: undefined }, null, WAREHOUSE),
				/Distribution return stock ledger requires distribution_batch ref_id/
			);

			// LOT invariants: missing/null or invalid prefix
			expectForbidden(
				() => compile()({ ...valid, lot_ref: undefined }, null, WAREHOUSE),
				/Distribution return stock ledger requires physical stock_ledger lot_ref/
			);
			expectForbidden(
				() => compile()({ ...valid, lot_ref: null }, null, WAREHOUSE),
				/Distribution return stock ledger requires physical stock_ledger lot_ref/
			);
			expectForbidden(
				() => compile()({ ...valid, lot_ref: 'lot:01J' }, null, WAREHOUSE),
			...
			expectForbidden(
				() => compile()({ ...valid, lot_ref: 'distribution_batch:01J' }, null, WAREHOUSE),
				/Distribution return stock ledger requires physical stock_ledger lot_ref/
			);

			// QTY invariants: zero
			expectForbidden(
				() => compile()({ ...valid, qty: '0' }, null, WAREHOUSE),
				/Distribution return stock ledger qty must be a positive decimal string/
			);
			expectForbidden(
				() => compile()({ ...valid, qty: '0.0' }, null, WAREHOUSE),
				/Distribution return stock ledger qty must be a positive decimal string/
			);
			expectForbidden(
				() => compile()({ ...valid, qty: '0.0000' }, null, WAREHOUSE),
				/Distribution return stock ledger qty must be a positive decimal string/
			);

			// QTY invariants: negative
			expectForbidden(
				() => compile()({ ...valid, qty: '-1' }, null, WAREHOUSE),
				/Distribution return stock ledger qty must be a positive decimal string/
			);
			expectForbidden(
				() => compile()({ ...valid, qty: '-0.1' }, null, WAREHOUSE),
				/Distribution return stock ledger qty must be a positive decimal string/
			);
			expectForbidden(
				() => compile()({ ...valid, qty: '-1.5' }, null, WAREHOUSE),
				/Distribution return stock ledger qty must be a positive decimal string/
			);

			// QTY invariants: malformed / non-string
			expectForbidden(
				() => compile()({ ...valid, qty: 'abc' }, null, WAREHOUSE),
				/Distribution return stock ledger qty must be a positive decimal string/
			);
			expectForbidden(
				() => compile()({ ...valid, qty: '' }, null, WAREHOUSE),
				/Distribution return stock ledger qty must be a positive decimal string/
			);
			expectForbidden(
				() => compile()({ ...valid, qty: '1.' }, null, WAREHOUSE),
			...
			expectForbidden(
				() => compile()({ ...valid, qty: 2.5 as unknown as string }, null, WAREHOUSE),
				/Distribution return stock ledger qty must be a positive decimal string/
			);

			// QTY invariants: precision violation (> 4 decimals)
			expectForbidden(
				() => compile()({ ...valid, qty: '0.00001' }, null, WAREHOUSE),
				/Distribution return stock ledger qty must be a positive decimal string/
			);
			expectForbidden(
				() => compile()({ ...valid, qty: '1.00001' }, null, WAREHOUSE),
				/Distribution return stock ledger qty must be a positive decimal string/
			);
			expectForbidden(
				() => compile()({ ...valid, qty: '0.12345' }, null, WAREHOUSE),
				/Distribution return stock ledger qty must be a positive decimal string/
			);
	...
	describe('requisition_ticket lifecycle and role rules (CR-121/CR-126, kitchen slice)', () => {
		...
		it('rejects a new ticket for an unimplemented requisition_type', () => {
			expectForbidden(
				() => compile()(newTicket({ requisition_type: 'food' }), null, KITCHEN),
				/only requisition_type "kitchen" is implemented/
			);
		});
		...
		it('rejects allocation from a non-warehouse role', () => {
			const ticket = newTicket();
			const allocated = newTicket({
				items: [{ ...(ticket.items as Doc[])[0], allocated_qty: '30' }]
			});
			expectForbidden(
				() => compile()(allocated, ticket, KITCHEN),
				/Only warehouse staff or system admin can allocate/
			);
		});
