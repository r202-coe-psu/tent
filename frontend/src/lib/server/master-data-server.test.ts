...
function shelterDoc(shelterCode: string): MasterData {
	return {
		_id: `master_data:housing_type:${shelterCode}`,
		type: 'master_data',
		schema_v: 3,
		master_type: 'housing_type',
		shelter_code: shelterCode,
		items: [
			{ code: 'dog', label_th: 'สุนัข', label_en: 'สุนัข', is_default: true, status: 'active' }
		],
...
describe('readShelterMasterDocs', () => {
	it('percent-encodes the JSON range keys instead of interpolating them raw', async () => {
		...
		await readShelterMasterDocs('housing_type');
		...
		expect(path).toContain('startkey=%22master_data%3Ahousing_type%3A%22');
		expect(path).toContain('endkey=%22master_data%3Ahousing_type%3A%EF%BF%B0%22');
	...
	it('scans only ids carrying a shelter suffix, never the global doc', async () => {
		...
		// The range starts AFTER `master_data:housing_type` + ':' so the global doc
		// (`master_data:housing_type`, no suffix) falls outside it.
		const startkey = decodeURIComponent(requestedPath().match(/startkey=([^&]+)/)![1]);
		expect(JSON.parse(startkey)).toBe('master_data:housing_type:');
	});
	...
	it('returns the docs of every shelter that has a local list', async () => {
		...
		const docs = await readShelterMasterDocs('housing_type');
		...
		adminRawMock.mockResolvedValue({
			...
			data: {
				rows: [
					...
					{ doc: null },
					{},
					{ doc: { _id: 'master_data:housing_type:SH003' } }
				]
			}
		});
	...
	it('returns an empty list when CouchDB answers without rows', async () => {
		...
		await expect(readShelterMasterDocs('housing_type')).resolves.toEqual([]);
	});
	...
	it('throws ServiceError on a CouchDB error so the write is not let through', async () => {
		...
		await expect(readShelterMasterDocs('housing_type')).rejects.toBeInstanceOf(ServiceError);
	});
});
