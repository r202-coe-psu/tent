...
export type CouchDocumentConflictMetadata = {
	_conflicts?: string[];
};

export async function getDocWithConflicts<T extends { _id: string }>(
	dbName: string,
	id: string,
	init?: CouchFetchInit
): Promise<(T & CouchDocumentConflictMetadata) | null> {
	try {
		return await couchDbFetch<T & CouchDocumentConflictMetadata>(
			dbName,
			`/${encodeURIComponent(id)}?conflicts=true`,
			init
		);
	} catch (err) {
		if (err instanceof NotFoundError) return null;
...
export interface PutDocOptions {
	onConflict?: 'return-existing' | 'throw';
}
...
export async function putDoc<T extends { _id: string; _rev?: string; type?: string }>(
	...
	doc: T,
	init?: CouchFetchInit,
	options: PutDocOptions = {}
): Promise<T> {
	const isCreate = !doc._rev;
	const onConflict = options.onConflict ?? 'return-existing';
	try {
		...
		if (err instanceof ConflictError && isCreate && onConflict === 'return-existing') {
...
export async function putDocStrict<T extends { _id: string; _rev?: string; type?: string }>(
	dbName: string,
	doc: T,
	init?: CouchFetchInit
): Promise<T> {
	return putDoc(dbName, doc, init, { onConflict: 'throw' });
}
...
export async function saveBulkAtomic<T extends { _id: string; _rev?: string }>(
	...
	label: string,
	init?: CouchFetchInit,
	options: { onConflict?: 'refresh' | 'throw' } = {}
): Promise<T[]> {
	...
	while (attempts < MAX_BULK_RETRIES) {
		try {
			...
			if (error instanceof Error && error.message === '409_CONFLICT') {
				// A newly-created immutable version must be regenerated by its repository
				// after a conflict. Refreshing it with an _rev would overwrite the winner.
				if (options.onConflict === 'throw') throw error;
				attempts++;
