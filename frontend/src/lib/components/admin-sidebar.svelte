<script lang="ts">
	...
	import {
		...
		adminHomePath,
		isGroup,
		type AdminSidebarNode
	} from './admin-sidebar/static';
	...
	function isActive(href: AdminSidebarNode['href']): boolean {
	...
	function groupIsActive(node: AdminSidebarNode): boolean {
...
>
	<div class="flex items-center justify-between border-b border-sidebar-border p-5">
		<a href={adminHomePath} class="flex items-center gap-3" aria-label="กลับหน้าเลือกเมนูหลัก">
			<div
	...
	<div
		class="no-scrollbar flex-1 space-y-6 overflow-y-auto p-4 text-[13px] font-medium text-muted-foreground"
	>
		<a
			href={adminHomePath}
		...
		{#each adminSidebarGroups as group (group.title)}
			<div>
				{#if !collapsed}
					<div
	...
	<div class="border-t border-sidebar-border bg-muted/50 p-4">
		{#if !collapsed}
			<div class="mb-1.5 text-[11px] font-bold tracking-wide text-muted-foreground/70 uppercase">
				จำลองสิทธิ์ผู้ใช้งาน (RBAC)
			</div>
			<div class="relative">
				<select
					class="w-full cursor-pointer appearance-none rounded-xl border border-sidebar-border bg-card px-3 py-2 pr-8 text-xs font-semibold text-foreground shadow-sm focus:ring-2 focus:ring-ring focus:outline-none"
				>
					<option>🛠️ ทีม IT / Data (Admin)</option>
				</select>
				<div
					class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3 text-[10px] text-muted-foreground"
				>
					<ChevronDown class="h-3 w-3" />
				</div>
			</div>
		{:else}
			<div class="flex justify-center" title="RBAC">
