<script lang="ts">
	...
	const formattedLabel = $derived.by(() => {
		...
		// Get the category axis label value from the raw tooltip data
		const dataLabel =
			tooltipData != null
				? chartCtx.valueAxis === 'x'
					? chartCtx.y(tooltipData)
					: chartCtx.x(tooltipData)
				: undefined;
...
<TooltipPrimitive.Root variant="none">
	<div
		bind:this={ref}
		class={cn(
			'min-w-9rem grid items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl',
			className
		)}
		{...restProps}
	>
