...
Static SPA frontend สำหรับ **Tent** (`เต็นท์`). สร้างจาก template **sveltekitten**
...
Static SPA — `@sveltejs/adapter-static` กับ `ssr = false`, `prerender = true`.
ไม่มี server-side code (ไม่มี `+page.server.ts` / load function ฝั่ง server). ดึงข้อมูล client-side หมด.
