...
Staff features talk to CouchDB (remote-first). The **public plane** talks to FastAPI via
same-origin gateway `/public-api` + OpenAPI paths `/public/v1/*`. End-to-end local stack and
“do not bypass” rules: **`CONTRIBUTING.md` §4.2**.
...
| `src/lib/api/public-client.ts`    | `openapi-fetch` client (`baseUrl: '/public-api'`, same-origin) |
...
```ts
// data/public-api.ts — prefer this pattern; never raw fetch past /public-api for typed routes
...
- When adding a new FastAPI public route: (1) implement + test in `backend/`, (2) keep the
  `/public-api` gateway (Vite/nginx already strip-prefix — no per-route proxy), (3)
  `pnpm openapi:update`, (4) wrap in `public-portal` data/application, (5) wire UI.
