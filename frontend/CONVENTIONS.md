...
## 12. Public plane (BFF ↔ FastAPI)

Staff features talk to CouchDB (remote-first). The **public plane** talks to FastAPI only via
same-origin SvelteKit BFF `/api/public/v1/*` (Bearer `EXTERNAL_API_SECRET`). End-to-end local
stack and “do not bypass” rules: **`CONTRIBUTING.md` §4.2** (CR-063).
...
| `src/lib/server/fastapi.ts`       | BFF → FastAPI base URL + `fastapiServiceHeaders()`          |
| `src/routes/api/public/v1/**`     | BFF proxies (inject Bearer; never expose secret to browser) |
| `src/lib/features/public-portal/` | Layered feature that calls BFF from the browser             |

Regenerate OpenAPI after FastAPI schema changes (backend must be on `:9000`):
...
features/public-portal/
  domain/        types + pure mappers (status, card shape)
  data/          public-api.ts — thin wrappers around fetch('/api/public/v1/…')
...
export async function familySearch(query: string) {
	...
	if (!response.ok || !data) throw new Error(/* unwrap error */);
	return data;
}
```
...
- Derive request/response TypeScript from `openapi.d.ts` when useful — don’t duplicate DTOs by hand
  unless you need a UI-only view model (then map in `domain/`).
- Adapt Mongo/API shapes in **domain mappers**. Do not reshape inside `.svelte` files.
- Routes and other features import only `$lib/features/public-portal` (barrel).
- Never call FastAPI from the browser (`/public-api` gateway removed). External agencies use
  `/external/v1` + managed API key.
- When adding a new FastAPI public route: (1) implement + test in `backend/` with
  `verify_external_secret`, (2) add BFF `+server.ts` under `api/public/v1/`, (3)
  `pnpm openapi:update` if types change, (4) wrap in `public-portal` data/application, (5) wire UI.
