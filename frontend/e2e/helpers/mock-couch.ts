...
function registryShelterSh001(enableMedicalScreening = false) {
	return {
		_id: 'shelter:SH001',
		_rev: '1-mock',
		type: 'shelter',
		...
		capacity: 100,
		feature_flags: {
			enable_medical_screening: enableMedicalScreening
		},
		zones: [
			{
				code: 'A',
...
export interface MockCouchRoutesOptions {
	...
	/** Shelter feature flag for Station 2 Medical Screening (#252). */
	enableMedicalScreening?: boolean;
	/** Pre-seed docs into the in-memory shelter DB (evacuee/household/…). */
...
export async function mockCouchRoutes(
	page: Page,
	{
		withRegistryShelter = false,
		enableMedicalScreening = false,
		seedDocs = []
	}: MockCouchRoutesOptions = {}
): Promise<void> {
	...
	if (withRegistryShelter) {
		const shelter = registryShelterSh001(enableMedicalScreening);
		await page.route('**/registry/_all_docs**', async (route) => {
			await route.fulfill({
				status: 200,
				contentType: 'application/json',
				body: JSON.stringify({
					rows: [{ id: shelter._id, doc: shelter }]
				})
			});
		});
	}
	...
	for (const doc of seedDocs) {
		...
		if (!id) continue;
		revCounter += 1;
		docs.set(id, { ...doc, _rev: doc._rev ?? `${revCounter}-mock` });
	}
	await page.route(`**/${SHELTER_DB}/**`, async (route) => {
		...
		if (last === '_find') {
			...
			await route.fulfill({
				status: 200,
				contentType: 'application/json',
				body: JSON.stringify({ docs: matched, bookmark: 'nil' })
			});
			return;
		}
