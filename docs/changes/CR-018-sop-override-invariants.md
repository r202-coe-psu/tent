...
# CR-018 — SOP master/override invariants and resolve semantics
...
1. `sop_override.ratios` ต้องเป็น complete snapshot ครบทุก canonical key; การแก้ override =
   ส่งทั้ง document (full replacement) — ปฏิเสธ partial patch
2. `sop_profile` (master) แก้แบบ partial ได้ (≥1 key); `ratios = {}` (ว่าง) → reject;
   key นอก whitelist → reject
3. ต่อ 1 `shelter_code` มี `sop_override` ที่ `active = true` ได้ไม่เกิน 1 ตัว; กลไกการ enforce
   เป็นรายละเอียด implementation
4. `sop_override.base_profile_id` immutable หลัง create; ตรวจ referential integrity ตอน create
...
7. `version` ต้องเพิ่มแบบ monotonic; วิธีนับ = **integer เรียง `prev.version + 1`** (resolved โดย
   [CR-026](CR-026-sop-ratio-catalog-scope-and-history-ratification.md), approved 2026-07-03; เดิม deferred)
...
- create override ที่อ้าง `base_profile_id` ที่ไม่มีอยู่ → reject
...
- **Stable core:** ข้อ 3/4/5 ต้องถูก enforce ผ่าน validation layer ที่เลือกตอน implement
  (`validate_doc_update` = preferred location) — ไม่ล็อก architecture; การ enforce active-uniqueness
  แบบ atomic (ข้อ 3) = deferred to implementation phase
