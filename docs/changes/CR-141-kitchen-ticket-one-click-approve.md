---
id: CR-141
title: ตั๋วเบิกวัตถุดิบครัว (requisition_ticket ประเภท kitchen) อนุมัติจบในคลิกเดียว — ยกเลิกการแยก 3 บทบาทของ CR-139 §2.2 เฉพาะตั๋วประเภทนี้
status: approved
date: 2026-09-25
updated: 2026-09-25
...
layer: volatile
supersedes:
  - CR-139 §2.2 ("Role แยกเต็มรูป ไม่มี self-approve/bypass") — เฉพาะส่วนที่บังคับ 3 บทบาทแยกกัน
    (ผจก.อนุมัติ / คลังปล่อยของ / ครัวยืนยันรับ) สำหรับ `requisition_type: 'kitchen'` เท่านั้น
    ส่วนอื่นของ CR-139 (lifecycle 4 สถานะยังมีอยู่ในข้อมูลเก่า, backward-compat union,
...
# CR-141: ตั๋วเบิกวัตถุดิบครัว อนุมัติจบในคลิกเดียว

## 1. Why

CR-139 §2.2 ("Role แยกเต็มรูป ไม่มี self-approve/bypass") เคาะไว้ว่าตั๋วครัวต้องผ่าน 3 บทบาทแยกกัน
...
shelter_manager) เจ้าของโครงการ (ผู้เคาะ CR-139 เดิม) ตัดสินใจกลับคำเฉพาะจุดนี้: ให้กดอนุมัติครั้งเดียว
จบเลย ไม่ต้องกดแยกทีละขั้น — ระบุชัดว่ายกเลิกเฉพาะข้อ 2.2 ของ CR-139 (ไม่แตะข้ออื่น)
...
- CR-140 (แก้ไขตั๋วขณะ PENDING_PICK) ยังใช้ได้ตามเดิม — ถ้าตั๋วเปลี่ยนเป็น COMPLETED ไปแล้วในคลิกเดียว
