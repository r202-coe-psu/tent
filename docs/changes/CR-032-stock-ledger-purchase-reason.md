...
affects:
  ...
  - UI surface ใหม่ของ purchase (สร้างใบ + key รับเข้า) — route `(protected)/back-office/purchases/` (เคาะ 2026-07-25 ดู §UX decisions)
  - frontend/scripts/seed.ts (purchase demo 1 ใบ + receipt)
  - frontend/src/lib/server/shelter-access-design.ts + .test.ts (`purchase` เข้า allowlist ของ `validate_doc_update` — §Change ข้อ 6, blocker)
...
6. **เพิ่ม `'purchase'` เข้า allowlist ของ `validate_doc_update` ใน shelter db** (added 2026-07-25 — ดู Decision log) · `frontend/src/lib/server/shelter-access-design.ts` (`buildValidateDocUpdate` → `var allowed = [...]`) + assertion ใน `shelter-access-design.test.ts`
   > **นี่คือ blocker ของ CR นี้ ไม่ใช่ nice-to-have** — shelter db ที่ provision ผ่าน `/api/back-office/shelter` มี `_design/access` ที่ปฏิเสธ `type` ที่ไม่อยู่ใน allowlist (`forbidden: doc type not allowed yet: purchase`) ⇒ ถ้าไม่แก้ข้อนี้ **ทุก write ของ `purchase` จะพังบน shelter จริง** แม้ UI เสร็จครบ · dev ที่ใช้ `seed.ts` ไม่เจอ เพราะ seed deploy `_design/access` ให้แค่ **catalog db** ไม่ deploy ให้ shelter db · ต้อง **redeploy design doc ให้ shelter db ที่มีอยู่แล้ว** ด้วย (ดู Migration)
...
- **Role/permission:** สร้าง `purchase` doc + เขียน ledger `reason:purchase` = **เหมือน FR-28 receive เป๊ะ** — `warehouse_staff` เป็นผู้เขียน (+ `system_admin` global); **SM ไม่เขียน ledger ตรง** ตาม operating note §3 (แม้ SM ⊇ WS) · shelter-scoped (ข้ามศูนย์ = NoPermission) · internal-only (ไม่มี public/donor tier) · gate ทั้ง purchase doc + ledger row ที่ **route guard (`requireWarehouseAccess`) + Zod** (~~gate 2 write แบบ atomic~~ ⚠️ ถ้อยคำเดิมแก้ 2026-07-25: Option A เป็น 2 action แยกกัน ไม่มี atomic gate ระดับ DB — อย่าเคลม) · ตัดสิน 2026-07-24 (ดู role-permission-matrix §3)
  > ⚠️ **แก้ข้อเท็จจริง 2026-07-25:** ถ้อยคำเดิม *"โปรเจกต์ไม่มี `validate_doc_update` จึงไม่มี enforcement ระดับ CouchDB"* **ไม่ตรงกับ code** — shelter db **มี** `_design/access` (`lib/server/shelter-access-design.ts`) ที่บังคับ **envelope §0 + `shelter_code` ต้องตรงกับ db + allowlist ของ `type`** · สิ่งที่ *ไม่มี* คือ enforcement ระดับ **RBAC/role** และระดับ **field invariant** — CouchDB ไม่รู้ว่าใครเป็น `warehouse_staff` (แยกสิทธิ์ด้วย `_security` roles ต่อ db เท่านั้น) ⇒ **ข้อสรุปเดิมยังถูก** ว่า role gating อยู่ที่ route guard + Zod แต่ห้ามพูดว่า "ไม่มี `validate_doc_update`" เพราะมันมี และมันคือเหตุที่ต้องเพิ่ม §Change ข้อ 6
- **Provisioning (เพิ่ม 2026-07-25):** `_design/access` ของ shelter db ต้อง allow `type: 'purchase'` (§Change ข้อ 6) — เป็น **การเปลี่ยน provisioning artifact** ไม่ใช่แค่ code ในแอป: shelter db ที่ provision ไปแล้วถือ design doc รุ่นเก่าอยู่ จึงต้อง redeploy (ดู Migration) · ทางแก้ไม่แตะ `_security` / roles ⇒ **ไม่กระทบเรื่องสิทธิ์** ใครเขียนได้เท่าเดิม
...
- **`_design/access` ต้อง redeploy (เพิ่ม 2026-07-25 · §Change ข้อ 6)** — shelter db ที่ provision ไปแล้วถือ allowlist รุ่นเก่าที่ไม่มี `purchase` · ใช้ `redeployShelterAccessDesign(db, shelterCode)` (`lib/server/shelters.admin.ts`) ซึ่งเป็น idempotent re-PUT (GET `_rev` → PUT ทับ)
  > **มี precedent อยู่แล้ว:** helper ตัวนี้ถูกเขียนขึ้นเพื่อสถานการณ์นี้เป๊ะ — doc comment ระบุ *"Use after whitelist changes (e.g. CR-034 `audit` type) when provisioning did not re-run"* ⇒ CR-032 เดินตามขั้นตอนเดียวกับ **CR-034** ที่เคยเพิ่ม `audit` เข้า allowlist
  > **ข้อจำกัดที่ต้องรู้:** ยังไม่มี HTTP route ไหนเรียก helper นี้ (grep `src/routes` ไม่เจอ) ⇒ ต้องรันจาก dev script / admin action ที่จะทำเพิ่ม · shelter ที่ provision **หลัง** merge ได้ allowlist ใหม่เองจาก `/api/back-office/shelter` ไม่ต้องทำอะไร
...
- 2026-07-25 — **amend: §Change ข้อ 6 (`purchase` เข้า allowlist ของ `validate_doc_update`) + แก้ข้อเท็จจริงเรื่อง `validate_doc_update`** · ⚠️ **CR นี้ `approved` แล้ว การ amend นี้เพิ่ม requirement ใหม่ → ควรแจ้ง PM/owner ให้รับทราบก่อน merge slice 3/3** · **ที่มา:** พบระหว่างเขียน report ของ CR-045 — ตรวจ `lib/server/shelter-access-design.ts` แล้วเจอว่า `var allowed = [...]` **ไม่มี `'purchase'`** ⇒ shelter db ที่ provision จริงจะปฏิเสธทุก write ของ `purchase` (`forbidden: doc type not allowed yet: purchase`) · **unit test ไม่จับ** เพราะ mock ที่ชั้น repository (628 tests ผ่านหมด) และ `seed.ts` deploy `_design/access` ให้แค่ catalog db ⇒ dev ที่ seed ไม่เจอ **แต่ shelter จริงพัง** · **สาเหตุที่หลุดมาถึงตอนนี้:** เอกสารหลายที่ (CR นี้ §Impact + `prd/role-permission-matrix.md` §3) เคลมว่า *"โปรเจกต์ไม่มี `validate_doc_update`"* ซึ่งไม่จริง — พอเชื่อว่าไม่มี จึงไม่มีใครไปดู allowlist · ถ้อยคำที่ถูก: **มี** `_design/access` ที่บังคับ envelope §0 + `shelter_code` + allowlist ของ `type`; สิ่งที่ไม่มีคือ enforcement ระดับ **role** และระดับ **field invariant** ⇒ ข้อสรุปเดิม (role gate อยู่ที่ route guard + Zod) **ไม่เปลี่ยน** · **ripple:** แก้ถ้อยคำใน CR นี้ (§Impact Role/permission + entry 2026-07-25 Option A ด้านล่าง) และ `prd/role-permission-matrix.md` §3 · เพิ่ม §Impact bullet **Provisioning** + ขั้นตอน redeploy ใน §Migration (ใช้ `redeployShelterAccessDesign` — precedent เดียวกับ **CR-034** ที่เพิ่ม `audit`) · **ยังไม่แก้ code** — รอ owner รับทราบ amend นี้ก่อน (ลำดับตาม `change-management.md`: บันทึก CR ก่อน แล้วค่อยแก้) · **finding ที่ยังไม่ได้ตัดสิน:** allowlist ขาดอีก **8 type** ที่ feature ปัจจุบันเขียนลง shelter db จริง — `household` / `medical` / `movement` / `screening` (people) · `kitchen_requisition` / `meal_plan` / `meal_service` (kitchen) · `daily_calc` (resource-calc) ⇒ เป็น gap ที่มีมา**ก่อน** CR-032 และเป็นการตัดสิน authz คนละเรื่อง **จึงไม่รวมใน CR นี้** — ต้องยก CR แยก
...
- 2026-07-25 — **follow-up CR ที่แตกออกไป:** design review ของ Option A พบว่า invariant `reason` ↔ `ref_id` ไม่ถูกบังคับที่ไหนเลย (`stockLedgerInputSchema` รับสตริงอะไรก็ได้) และกับดัก 3 (ช่อง `ref_id` free text ในฟอร์มรับสต็อก) **ยังคาอยู่กับ `donation` / `transfer_in`** แม้ purchase จะเลี่ยงได้แล้ว → เปิด **[CR-055](CR-055-stock-ledger-refid-invariant.md)** (`approved` 2026-08-14 · เดิมยกไว้เป็น CR-045 → renumber เป็น CR-050 เมื่อ 2026-07-26 เพราะเลข 045 ถูกใช้ซ้ำ → renumber เป็น CR-055 อีกรอบเพราะ CR-050 ถูกใช้ซ้ำโดย evacuee-special-needs) แยกต่างหาก เพราะกระทบทุกแหล่ง ไม่ใช่แค่ purchase · **ไม่บล็อก CR-032** — `keyPurchaseReceipt` มินต์ `ref_id` จาก doc จริงจึงถูกต้องโดยโครงสร้างอยู่แล้ว
