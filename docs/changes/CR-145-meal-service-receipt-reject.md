...
layer: stable
extends:
  - CR-144 (meal_service_receipt — เดิมมีแค่ "ยืนยันตรวจรับ" ทางเดียว)
affects:
...
# CR-145: ปฏิเสธรับมอบ (ตีกลับโรงครัว)

## 1. Why

CR-144 ให้คลังกด "ยืนยันตรวจรับ" ได้ทางเดียว แต่ตัวอย่างหน้าจอที่เจ้าของโครงการอ้างอิงมี
...
append-only doc เดียวกับ CR-144 หนึ่งใบต่อ `meal_service` (idempotent ทั้ง 2 ทาง)
