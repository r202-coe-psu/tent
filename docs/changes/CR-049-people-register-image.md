...
**After (schema_v: 4 — รวม reconcile ช่องว่างเดิมเข้ากับการเพิ่ม field ใหม่ในครั้งเดียว):**
...
| `content_type` | str | req | mime type ของไฟล์ที่ compress แล้ว (`image/jpeg`) |
...
**Attachments:** `full` (JPEG compressed ≤1024px, quality 0.82), `thumb` (JPEG square-crop 200px) —
...
- [ ] เลือกไฟล์รูปที่หน้าลงทะเบียน Stage 3 แล้วเอกสาร `image:{ulid}` ถูกสร้างจริงใน CouchDB พร้อม
      attachment `full` + `thumb`
- [ ] หลังลงทะเบียนสำเร็จ เอกสาร `evacuee` มี `photo` ชี้ไปยัง `image` doc ที่สร้างไว้
- [ ] อัปโหลดรูปล้มเหลว (เช่น offline) ไม่บล็อกการกดถัดไป/ลงทะเบียน — แสดง toast error เฉยๆ
- [ ] `pnpm check` ผ่านไม่มี error จาก `frontend/src/lib/features/images/`
- [ ] `pnpm test` ผ่านสำหรับ `people.test.ts` (schema_v 4 + `photo`)
