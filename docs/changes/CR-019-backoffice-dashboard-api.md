...
- **แยก Feature Domain:** จัดโครงสร้าง `$lib/features/occupancy-dashboard` (หลังบ้าน) ออกมาจัดการ Zod Schema และ Business Logic โดยเฉพาะ

## 2. ข้อมูลที่ API ต้องส่งกลับ (API Payload Requirements)

API (`GET /api/back-office/shelter/[code]/occupancy-dashboard`) จะต้องส่งคืนสถิติเหล่านี้ (โดยดึงจาก CouchDB Views):
