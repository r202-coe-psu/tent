...
# CR-118 — T-13 โอนย้ายข้ามศูนย์: Lot ต้นทาง/ปลายทาง + Split Allocation
...
- 2026-09-06 — **approved** — Project Owner อนุมัติสเปกตามแนวทาง advisory per-lot · รันเลข `CR-114`
  (นับต่อจาก `CR-112` และ `CR-113` ของสาขา `feat/registration` ที่ approved แล้ว)
- 2026-09-13 — **renumber to CR-118** — เนื่องจากระหว่างรอ merge มี CR-114 ถึง CR-117 ถูกอนุมัติและ
  merge เข้า `develop` ก่อนหน้า (`CR-114-primary-contact-ui-label-for-head-evacuee.md`, `CR-115`, `CR-116`,
  `CR-117`) จึงรันเลขใหม่เป็น `CR-118` ตาม Policy §3 และบันทึกลง `_index.md` พร้อม merge เข้า `develop`
