...
- 2026-09-04 — **decision sync** (ไม่ใช่กฎใหม่ — บันทึกสิ่งที่ CR ลูกเคาะไปแล้วให้ CR แม่ตรงกัน;
  tracking = decision sync + หนึ่งประโยคใน bullet §4.3 ตามที่ project owner เคาะ) — เส้นทางออกจาก
  สถานะ `disputed` มีทางเดียวคือ resume กลับ `requested` · ที่มา: **CR-089 FR-07** (approved 2026-08-31,
  `done` 2026-09-02) กำหนดว่า `disputed` เข้าได้จาก `requested` เท่านั้นและออกได้กลับไป `requested`
  เท่านั้น — ไปที่ `shipped`/`received`/`cancelled` ตรงจาก `disputed` ไม่ได้ · และ **CR-090 FR-01**
  (approved 2026-09-01) จำกัดการลบไว้ที่ `requested` เท่านั้นเช่นกัน
  ⇒ ผลรวมที่ผู้ใช้เห็น: คำร้องที่ระงับไว้ต้องกด 2 ครั้งเสมอถ้าจะเอาออก — "กลับมาดำเนินการต่อ" แล้วจึง
  "ยกเลิก" หรือ "ลบ" · **ยืนยันว่าเป็นพฤติกรรมที่ต้องการ** (project owner, 2026-09-04): การระงับควรต้อง
