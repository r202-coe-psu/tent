...
affects:
  ...
  - CR-089, CR-090 (สืบเนื่องจากการแยก CR — ไม่ต้องรอทั้งสองอัน approve ก่อน, เป็นอิสระจากกัน)
---
...
> approve/ship อิสระจาก CR-089/CR-090 ได้ทั้งหมด (แสดง field จาก CR-089/CR-090 แบบ progressive — field
...
- **FR-03** — แสดงรายการ item ทั้งหมดของคำร้อง พร้อม field ที่มีอยู่แล้ว ณ วันที่หน้านี้ ship
  (`driver_name`, `vehicle_plate` — field ไหนยังไม่มีในโค้ดเพราะ CR-089 ยังไม่ landed ให้ข้ามการแสดงผล
  เงียบๆ ไม่ throw หรือขึ้น error) — **ไม่มี `lot`/`source_lot`**: CR-089 ตัดกลุ่ม Lot ออกแล้ว (ซ้ำกับ
  CR-088) จึง `StockTransferItem` ยังไม่มี field นี้เลยในรอบนี้
...
- **FR-05** — ตาราง `transfer-list.svelte` เพิ่มลิงก์ "ดูรายละเอียด" ต่อแถว ไปหน้าใหม่นี้ — ปุ่ม action
  ...
  **ยังอยู่ที่ตาราง list เหมือนเดิม ไม่ย้าย** ในรอบนี้ — การยุบปุ่มเข้าหน้ารายละเอียดเพื่อลดความแน่นของแถว
  เป็น follow-up ที่ทำได้ทีหลังเมื่อทุก CR ที่เกี่ยวข้อง (CR-089, CR-090) landed ครบแล้ว ไม่ใช่ scope ของ
  CR นี้
...
- ออกแบบให้ field จาก CR-089/CR-090 เป็น progressive enhancement (แสดงถ้ามี, ข้ามถ้ายังไม่มี) เพื่อให้
