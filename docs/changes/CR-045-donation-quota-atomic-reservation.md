...
- `cancel()` และ worker retention job (`purge_expired_buffers`) ต้อง decrement ตัวนับคืนเมื่อ
  ยกเลิก/หมดอายุ
- **Underflow Guard:** การ decrement คืนโควตาทุกจุด (cancel, expire, rollback) ต้องใส่ constraint
  `reserved_qty ≥ qty` (หรือ clamp ค่าไม่ให้ติดลบ `$gte: 0`) เพื่อป้องกันการคลาดเคลื่อนสะสม
- **Compensation Rollback:** ถ้าจองหลาย item ในใบเดียว แล้ว item หนึ่ง reject ระหว่างทาง ต้อง compensate
  (ลดตัวนับ item ที่ increment ไปแล้วคืน) เพราะ Mongo deployment ปัจจุบันเป็น single-node ไม่มี replica set
  — ไม่มี multi-document transaction ให้ใช้ all-or-nothing แบบ built-in
- **Shared Domain Helper (DRY):** สกัด logic การ decrement และ counter calculation ออกเป็น shared domain module
  เพื่อให้ทั้ง FastAPI (`donations`) และ Worker (`purge_expired_buffers`) เรียกใช้ร่วมกัน ป้องกัน logic ซ้ำซ้อน
...
- **One-time Backfill Script:** ก่อน rollout ต้องรัน script คำนวณ `reserved_qty` ตั้งต้นจาก `DonationBuffer`
  ที่มีสถานะ `declared`/synced `received` ปัจจุบัน ต่อ `(shelter_code, campaign_id, item_id)` มาตั้งค่าเริ่มต้น
  ของ `donation_need_counter` ก่อนเปิดใช้ atomic check
- **Cutover Lock:** การรัน Backfill Script บน Production ต้องกระทำใน Maintenance Window หรือล็อค Write path
  ชั่วคราว เพื่อป้องกันไม่ให้มี transaction ใหม่แทรกระหว่างประมวลผลยอดตั้งต้น
- **Recalculation CLI Tool (DR):** เพิ่ม CLI script / Admin command สำหรับสั่ง Recalculate ค่า `reserved_qty`
  จาก `DonationBuffer` ใหม่ได้ในกรณีเกิด Data Inconsistency หรือ DB Recovery

## Decision log

- 2026-07-24 — approved by project owner (updated with robustness guards: underflow protection, DR recalculation tool, cutover locking, DRY shared helper)
