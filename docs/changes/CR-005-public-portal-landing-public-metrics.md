...
affects:
  ...
  - docs/data/schema.md §2.3 donation (schema_v 1 → 2: +logistics, booking_ref, donor.line_id/email, items.category/condition/note) + §2.13 donation_slot (NEW — DN-5) — §F
  ...
  - docs/task-breakdown/ — +module 12 Public Portal (T-57/58/59); Donation +T-60 `/donate`; Family Search T-41 `/search` bump; T-01/T-02/T-35 notes; _index + _timeline totals 250 → 268.5 Adj MD
---
...
ขั้นต่ำ = ชื่อ + เบอร์). open points (DN-2/3/4/5/6/7 เคาะ 2026-06-22; DN-1 ติดไว้ก่อน):

| # | คำถาม | สถานะ |
| --- | --- | --- |
| DN-1 | OTP ยืนยันเบอร์ตอน submit (flag `public_otp_required`) เปิด default ไหม | **ON HOLD** — ติดไว้ก่อน: develop แบบไม่มี OTP (flag = off), รอเจ้าของยืนยันรอบสอง |
| DN-2 | เก็บ Line ID / email (donor PII เกินร่างเดิม) | **RESOLVED** — เก็บได้ ทั้งคู่ optional |
| DN-3 | ใบอนุโมทนาบัตร/ลดหย่อนภาษี — PII เพิ่ม (ที่อยู่/เลขผู้เสียภาษี) + ใครออกใบ | **RESOLVED** — ระบบ**ไม่รองรับ**; ตัด checkbox/field/flow ออกใบ + ลบ `tax_receipt_requested` จาก schema |
| DN-4 | สูตร "ขาด N"/"ล้นสต็อก %"/"ของยังรับได้ %" + เกณฑ์ "งดรับ" | **RESOLVED** — `qty_target` ของ campaign = cap ต่อรอบ; ใช้ view `needs_open` เดิม (ไม่เพิ่ม field); งดรับ = `needs_open ≤ 0`; กัน race ตอน submit ด้วย `NEED_FULL` |
...
(DN-2/5/6/7) reflect ใน `public-tier-flow-spec §DN`; DN-3 ใบอนุโมทนาตัดออก; DN-4 ใช้สูตร
`needs_open` เดิม (cap = `qty_target`); DN-1 develop แบบไม่มี OTP ไปก่อน (รอยืนยันรอบสอง).

> **Status (2026-06-22):** spec page `docs/features/public-tier-donation-spec.html` เขียนแล้ว
> (v0.3; 4 ข้อ confirmed; DN-2/3/4/5/6/7 resolved; DN-1 on hold = no OTP).
...
- **`docs/features/public-tier-flow-spec.html` §DN** — reflect การขยาย DN surface: donor Line/email optional (DN-2), logistics + slot booking; ใบอนุโมทนา (DN-3) ไม่รองรับ; ชี้ไป donation-spec
- API `GET /public/v1/needs` + `POST /public/v1/donations` + `GET`/`PATCH /public/v1/donations/{token}` — field set (donor PII, items, logistics, slot, ETA, courier tracking) ตาม §F; DN-1 develop แบบไม่มี OTP (flag off); DN-4 ใช้ view `needs_open` (§2.4) + index `(campaign_id)` เดิม — `POST /donations` ต้อง re-check `needs_open ≤ 0` atomic ตอน submit → `NEED_FULL` (มิเรอร์ `SLOT_FULL`)
- **`docs/data/schema.md`** — `donation` §2.3 bump **schema_v 1 → 2** (+`logistics`, `booking_ref`, `donor.line_id/email`, `items[].category/condition/note`) + migration note; **ไม่มี `tax_receipt_requested`** (DN-3 ไม่รองรับ); **`donation_slot` §2.13 NEW** (DN-5, ศูนย์ตั้งค่า slot/คิว); §7 index + `slot_availability` view. `config.public_otp_required` (DN-1) มีอยู่แล้ว default off — ไม่แตะ (develop แบบไม่มี OTP ไปก่อน)
- API `GET /public/v1/faq` + EOC FAQ setup screen — dependency ใหม่ (task แยก)
- **`docs/task-breakdown/` (applied 2026-06-22)** — task-breakdown มี public-tier scope แล้ว:
  - **`12-public.md` (NEW module 12 Public Portal, PUB tier)** — T-57 landing + real-time metrics (§A/B/C, OP-1..7), T-58 Public Shelter Dashboard `/shelters` (§D, OP-8/9), T-59 dynamic FAQ + EOC FAQ setup screen (OP-1); รวม 11.5 Adj MD. **หมายเหตุ:** T-56 ถูกจองโดย CR-003 (LPG, Module D) — public-tier จึงเริ่มที่ T-57
  - **`04-donation.md`** — +T-60 public `/donate` wizard (§F: needs board + logistics + slot + ticket; DN-1..7), Adj MD 20 → 25.5; T-15 DoD ชี้ไป T-60; +dependency T-02 (schema_v2)
  - **`11-famsearch.md`** — T-41 `/search` DoD ขยาย (FS-1 identifier search / FS-2 field set masked / FS-3 ชื่อ mask / FS-4 no demo), bump 7→9 Raw / 5→6.5 Adj
  - **`01-core.md`** — T-01 redaction เป็น tier-specific (FAM national_id masked + PUB aggregate whitelist หลัง flag); T-02 +donation schema_v2/`donation_slot`/`validate_doc_update`; T-35 +public read-model (occupancy_total/vulnerable_count/needs_open/slot_availability, polling OP-7)
  - **`_index.md` + `_timeline.md`** — เพิ่ม module 12 row, ยอดรวม 250 → **268.5 Adj MD**; dependency graph + gantt + scope note เพิ่ม T-57..60
  - **หมายเหตุ:** §B (ซ่อน nav/card อาสาบน public landing) **ไม่กระทบ** scope ภายในของ Module A — Volunteer (T-28/29) — ระบุไว้ใน T-57 DoD
...
- 2026-06-22 — **DN-1 — ON HOLD**: ติดไว้ก่อน — **develop แบบไม่มี OTP ไปก่อน** (flag `public_otp_required` = off), รอเจ้าของยืนยันรอบสอง (เจ้าของ "ให้ติดไว้ก่อน รอรับการยืนยันให้ develop แบบไม่มี otp ไปก่อน")
- 2026-06-22 — **DN-3 — RESOLVED**: ใบอนุโมทนาบัตร/ลดหย่อนภาษี **ระบบไม่รองรับ** (เจ้าของ "ไม่มีครับ ระบบนี้ไม่รองรับเรื่องนี้"); ตัด checkbox/field/flow ออกใบในหน้า `/donate` + ลบ `tax_receipt_requested` จาก schema.md §2.3
- 2026-06-22 — **DN-4 — RESOLVED**: campaign `qty_target` = cap ต่อรอบ (เจ้าของ "ตั้ง campaign กำหนดว่ารอบนี้รับเข้าเท่าไหร่; ครบ/เกินแล้ว donor คนต่อไป donate รายการนั้นไม่ได้ ต้องไปรายการถัดไป"). ใช้ view `needs_open` เดิม (schema.md §2.4) — **ไม่เพิ่ม field**: `needs_open = qty_target − Σ donation(declared+received)`; "ขาด N" = `max(0, needs_open)`; "ล้น %" = `Σ/qty_target×100`; เกณฑ์ "งดรับ" = `needs_open ≤ 0`. เพิ่ม error code **`NEED_FULL`** (มิเรอร์ `SLOT_FULL`) กัน race ตอน submit — service re-check atomic. reflect ใน donation-spec (Page States + UAT + DN-4 RESOLVED, v0.2 → v0.3)
- 2026-06-22 — **task-breakdown applied** (เจ้าของสั่ง "แก้ไข change นี้ และ tracking change ใน CR-005 ต่อไป"): เพิ่ม module 12 Public Portal (T-57/58/59, เริ่มที่ T-57 เพราะ T-56 ถูกจองโดย CR-003 LPG) ใน `12-public.md`; Donation +T-60 `/donate` (`04-donation.md`); Family Search T-41 `/search` DoD ขยาย + bump (`11-famsearch.md`); T-01 redaction tier-specific + T-02 schema_v2/`donation_slot` + T-35 public read-model (`01-core.md`); `_index.md`/`_timeline.md` ยอดรวม 250 → **268.5 Adj MD** + dependency/gantt. estimate provisional (recalibrate K-16); Team assignment ของ public-portal surface = open (workshop K-13 follow-up). §B ไม่กระทบ Module A scope ภายใน
- 2026-08-21 — **AMENDMENT (CR-041 D-PUBLIC=A)**: ปลดล็อก (Re-enable) เมนูและ Quick-service Card "ทีมอาสาสมัคร" (`/volunteer`, `/volunteer/jobs`) บนหน้า Public Landing Portal เพื่อรองรับ Public Volunteer Job Board & Application ตามผลการออกแบบ Module A
