...
เจ้าของโครงการได้เคาะข้อสรุปทั้ง 5 ข้อเมื่อ 2026-08-21 ดังนี้:

| # | ประเด็นคำถาม | ตัวเลือก / ข้อเสนอของทีม | มติที่อนุมัติ (Approved Decision) |
| - | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| 1 | แก้ได้ถึงสถานะไหน | ตอนนี้ code จำกัดที่ `declared` เท่านั้น (`DONOR_EDITABLE_STATUSES`). ถ้าอยู่ `pending_review` แล้วยังให้แก้ได้ไหม? ทีมเสนอ: **แก้ไม่ได้** เมื่อเจ้าหน้าที่เริ่มประเมินแล้ว | **แก้ไม่ได้** (อนุญาตเฉพาะสถานะ `declared` เท่านั้น เมื่อเข้า `pending_review` หรือเจ้าหน้าที่เริ่มตรวจแล้ว ห้ามแก้) |
| 2 | แก้แล้วโควตาไม่พอ (`NEED_FULL`) | (ก) reject ทั้งคำขอ ของเดิมไม่เปลี่ยน ← **ทีมเสนอ**<br>(ข) รับเท่าที่เหลือ<br>(ค) ตัดเฉพาะรายการที่เต็ม | **(ก) Reject ทั้งคำขอ (HTTP 409 `NEED_FULL`) แบบ All-or-Nothing** — คงรายการเดิมไว้ทั้งหมด ไม่เปลี่ยนบางส่วน |
| 3 | การนับ TTL ของการจอง | (ก) นับต่อจาก `declared_at` เดิม ← **ทีมเสนอ** (กัน abuse ต่ออายุไม่รู้จบด้วยการแก้รัวๆ)<br>(ข) รีเซ็ตใหม่ทุกครั้งที่แก้ | **(ก) นับต่อจาก `declared_at` เดิม** — ไม่อนุญาตให้รีเซ็ตเวลาใหม่ ป้องกันการกั๊กโควตา |
| 4 | Log ละเอียดแค่ไหน / เก็บที่ไหน | (ก) `revisions[]` ใน donation doc เก็บ items ก่อน-หลังทั้งชุด ← **ทีมเสนอ**<br>(ข) เก็บแค่ diff<br>(ค) แยกเป็น audit doc ต่างหาก | **(ก) ฝัง `revisions[]` ใน donation doc** — เก็บ snapshot `items_before` และ `items_after` ทั้งชุด (`schema_v` 3 → 4) |
| 5 | จำกัดจำนวนครั้งไหม | ตอนนี้ rate-limit ที่ BFF 3 ครั้ง/นาที ต่อ IP + FastAPI 30 ครั้ง/นาที ต่อ IP. ต้องมีเพดาน N ครั้งต่อการจองเพิ่มไหม? ทีมเสนอ: ไม่ต้อง | **ไม่ต้องจำกัดจำนวนครั้งต่อการจอง** — ใช้ Rate-limit ต่อ IP และตรวจสอบย้อนหลังผ่าน `revisions[]` |
