...
updated: 2026-06-11
module: eoc
note: generated by gen_modules.py from _tasks.py · Task Details (description/DoD) เพิ่ม manual 2026-06-10 — ถ้า regenerate ต้อง merge กลับ · architecture note เพิ่ม 2026-06-11
---
...
**Architecture (เคาะ 2026-06-11):** โมดูลนี้เป็น **service แยก** ออกจากระบบหลัก — worker/ETL อ่านข้อมูลจาก Central CouchDB (ระบบหลัก offline-first; LAN Edge เป็น fallback replica ไม่ใช่ API source) มาคำนวณสรุปเป็น aggregate read-model แล้ว expose ผ่าน EOC API + Open API tier; ระบบหลักไม่มี EOC dashboard/role. **Datastore ของ aggregate read-model = รอตัดสินใจใน P-03** — ร่างเดิมวาง MongoDB ไว้; ทางเลือก: CouchDB database แยก (ไม่เพิ่มชนิด infra) หรือ PostgreSQL (ถ้าต้องการ SQL aggregation) — เกณฑ์ตัดสิน: ops burden ของทีมหลังส่งมอบ
...
**Description:** Read-only API รวมข้อมูลข้ามศูนย์สำหรับ EOC อำเภอ/จังหวัด: ยอดผู้พักพิงต่อศูนย์, สถานะ capacity, ทรัพยากรขาด/เกิน, กลุ่มเปราะบาง (aggregate + selected fields เท่านั้น — **ไม่มี person-level drill-down**) ตาม FD-14

**Definition of Done:**
- Endpoint aggregate ครอบชุดข้อมูลที่ P-03 design กำหนด (occupancy, capacity, resource gap, vulnerable counts)
- ไม่มีทางดึง PII/person-level ผ่าน API ชุดนี้ (test ยืนยัน รวม parameter manipulation)
...
**Description:** Open API tier สาธารณะสำหรับหน่วยงานภายนอก (One Data / Hat Yai ROD) — **แยก tier จาก EOC API ตาม FD-14**: ชุดข้อมูล aggregate ที่เปิดกว้างกว่าแต่หยาบกว่า, มี auth + rate-limit + versioning ตาม Open API contract ใน P-03

**Definition of Done:**
- Endpoint ตาม contract P-03, versioned (`/v1/`) — breaking change ต้องออก version ใหม่
- Auth + rate-limit แยกจาก EOC tier, ข้อมูลเป็น aggregate ที่ผ่านเกณฑ์เปิดเผยสาธารณะเท่านั้น (ไม่มี PII — test ยืนยัน)
