...
**Architecture (เคาะ 2026-06-11):** โมดูลนี้เป็น **service แยก** ออกจากระบบหลัก — worker/ETL อ่านข้อมูลจาก CouchDB (ระบบหลัก offline-first) มาคำนวณสรุปเป็น aggregate read-model แล้ว expose ผ่าน EOC API + Open API tier; ระบบหลักไม่มี EOC dashboard/role. **Datastore ของ aggregate read-model = รอตัดสินใจใน P-03** — ร่างเดิมวาง MongoDB ไว้; ทางเลือก: CouchDB database แยก (ไม่เพิ่มชนิด infra) หรือ PostgreSQL (ถ้าต้องการ SQL aggregation) — เกณฑ์ตัดสิน: ops burden ของทีมหลังส่งมอบ
