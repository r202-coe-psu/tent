...
> DoD ยึดมาตรฐานกลาง (task-breakdown §9): **prod = UI + API + data + test + demo ของ slice · post = gate/handover artifact** — รายการด้านล่างคือเกณฑ์เฉพาะของ task นั้นเพิ่มจากมาตรฐานกลาง

### T-01 — RBAC extension: new roles + field-level permissions (FR-34)

**Description:** สร้างระบบ RBAC เต็มรูปต่อจาก auth/RBAC skeleton ของ walking skeleton (**greenfield — ไม่มีระบบ MVP เดิมรันอยู่**): ครอบ role ฝั่ง baseline (registration/screening/medical ตาม FR-1..3) + role ของ full system (inventory, kitchen, security, shelter_manager ฯลฯ ตาม [role-permission matrix](../prd/role-permission-matrix.md)) และ **field-level permission** สำหรับข้อมูลอ่อนไหว (medical, PII) — ทุกโมดูลพึ่ง task นี้ในการคุมสิทธิ์ เป็นส่วนหนึ่งของ walking skeleton ที่ Lead ต้องส่งก่อน 17/06

**Definition of Done:**
- Role + permission ครบตาม role-permission matrix, mapping ทดสอบ allow/deny ต่อ role อัตโนมัติ
- Field-level masking บังคับที่ API layer (เช่น role ที่ไม่มีสิทธิ์ medical เห็นค่า masked) — ไม่พึ่ง frontend ซ่อน
...
**Description:** วาง data model ของทั้งระบบ (**greenfield** — เริ่มจาก base schema ใน walking skeleton: person, screening, movement ตาม baseline FR-1–20) แล้วขยายแบบ **additive**: household, zone, supply/ledger, donation บน CouchDB รวมถึง sync/conflict strategy — **fan-out ใหญ่สุดของโครงการ** (block T-04/08/10/17/18/19) และเป็น tech risk #1 (CouchDB offline-first) เจ้าของคือ Lead B ตาม schedule decision

**Definition of Done:**
- Schema ทุก entity ตรง data dictionary + ERD ใน `docs/data/` พร้อม validation
- Sync/conflict design (offline-first) เขียนเป็นเอกสาร + พิสูจน์ด้วย test conflict scenario จริงบน CouchDB
...
**Description:** กำหนดมาตรฐาน API ใช้ร่วมทุกทีม (naming, error shape, pagination, auth, versioning) แล้ว **freeze contract ของ phase** — กันปัญหา 4 ทีมต่างคนต่างออกแบบจน integrate ไม่ได้ตอน gate
