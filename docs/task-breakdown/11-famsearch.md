...
**Description:** หน้าเว็บสาธารณะให้ญาติค้นหาผู้ประสบภัย (ไม่ต้อง login) — MVP ใช้ conservative search: exact/near-exact ชื่อ+นามสกุล, reject query สั้นเกิน 3 chars, ไม่มี list-all endpoint. ผลลัพธ์เปิดเผยขั้นต่ำ: ยืนยันว่า "พบ และอยู่ศูนย์ใด" โดย mask ข้อมูลส่วนตัวอื่น พร้อมกลไก anti-enumeration กันการกวาดข้อมูล

**Definition of Done:**
- ค้นจากชื่อ-นามสกุลแบบ exact/near-exact ตามเกณฑ์ P-03 → แสดงข้อมูลขั้นต่ำตาม PRD FR-53: "พบ/ไม่พบ" + ศูนย์ (ระดับที่ปลอดภัย) + ช่องทางติดต่อเจ้าหน้าที่ — **ไม่เปิด** medical, national ID, address, phone; ระดับการเปิดชื่อ masked ให้เป็นไปตาม P-03/DPIA
- เฉพาะคนที่ consent (T-40) เท่านั้นที่ปรากฏ; ไม่พบ กับ ไม่ยินยอม ต้องแยกไม่ออกจากภายนอก
- Anti-enumeration: rate-limit ต่อ IP/session, CAPTCHA ใน production public, query สั้นเกิน 3 chars หรือกว้างเกินถูกปฏิเสธ, ไม่มี endpoint ไล่ดึงรายชื่อ (test เชิง adversarial ครบ)
