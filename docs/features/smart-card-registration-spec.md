...
flowchart TD
    ...
    NewWalkin --> CreateDraft["สร้าง Evacuee ใหม่<br/>- ตั้ง status = 'draft'<br/>- บันทึกข้อมูลบัตรลง card_snapshot"]
    CreateDraft --> ScreenNew["หน้าจอ Kiosk แสดงข้อความ:<br/>'อ่านบัตรสำเร็จ กรุณาไปพบเจ้าหน้าที่เพื่อคัดกรองและยืนยันข้อมูล'"]

    CheckDB -- "2. มีข้อมูลจองล่วงหน้าอยู่แล้ว" --> PreReg["กรณี ลงทะเบียนล่วงหน้า (Pre-registered)"]
    PreReg --> AttachCard["อัปเดต Evacuee เดิม<br/>- คง status = 'pre_registered' (ไม่ลดเป็น draft)<br/>- แนบ card_snapshot และบันทึกเวลายืนยันบัตร"]
    AttachCard --> ScreenPre["หน้าจอ Kiosk แสดงข้อความ:<br/>'อ่านบัตรสำเร็จ พบข้อมูลการจองล่วงหน้า กรุณาไปพบเจ้าหน้าที่เพื่อคัดกรอง'"]

    CheckDB -- "3. เข้าพักในศูนย์แล้ว" --> Active["กรณี พักอาศัยอยู่แล้ว (Active)"]
    ...
    RenderResult -- "สถานะ: เสียบบัตรแล้ว (Draft)" --> FlowDraft["Flow ผู้ประสบภัยใหม่<br/>1. Step 1: คัดกรองสุขภาพ EWAR ใหม่<br/>2. Step 2 & 3: Autofill ข้อมูลส่วนตัวและที่อยู่จากบัตร<br/>3. ยืนยันข้อมูล -> เปลี่ยนสถานะเป็น Active"]

    RenderResult -- "สถานะ: จองล่วงหน้า + ยืนยันบัตรแล้ว" --> FlowPre["Flow จองล่วงหน้า (Fast-Track Check-in)<br/>1. Step 1: คัดกรองสุขภาพ EWAR หน้างาน<br/>2. Step 2 & 3: ตรวจสอบข้อมูลที่จองไว้เทียบกับบัตร<br/>3. ยืนยันข้อมูล -> ทำ Check-in เป็น Active ทันที"]
```
...
- **FR-CARD-02:** หาก CID 13 หลักยังไม่เคยมีในฐานข้อมูลศูนย์ ให้สร้าง doc `evacuee` ใหม่ โดยกำหนด:
  ...
  - `first_name`, `last_name`, `gender`, `birth_year`, `age`: Map จากชิปบัตร
  - `card_snapshot`: เก็บ snapshot ข้อมูลบัตรและที่อยู่ตามทะเบียนบ้าน
  - `household_id`: `null`
- **FR-CARD-03:** หาก CID 13 หลักมีสถานะเป็น `pre_registered` อยู่แล้ว ให้คงสถานะ `pre_registered` ไว้ พร้อมอัปเดตแนบ `card_snapshot` เข้า doc เดิม และประทับเวลา `card_verified_at`
- **FR-CARD-04:** หาก CID 13 หลักมีรายการสถานะ `draft` รออยู่แล้ว ให้ปฏิเสธการสแกนซ้ำ และ Kiosk แจ้งเตือน: _"มีข้อมูลการสแกนบัตรนี้รออยู่แล้ว กรุณาไปพบเจ้าหน้าที่"_
- **FR-CARD-05:** ข้อความตอบสนองหน้าจอ Kiosk:
  - ผู้ประสบภัยใหม่: _"อ่านบัตรสำเร็จ กรุณาไปพบเจ้าหน้าที่เพื่อคัดกรองและยืนยันข้อมูล"_
  - ผู้จองล่วงหน้า: _"อ่านบัตรสำเร็จ พบข้อมูลการจองล่วงหน้า กรุณาไปพบเจ้าหน้าที่เพื่อคัดกรอง"_
  - ผู้เข้าพักแล้ว: _"ท่านได้เช็คอินเข้าพักในศูนย์แล้ว"_

### 3.2 Staff Onsite Search & Registration Flow (`/onsite/people`)

- **FR-STAFF-01:** ช่องค้นหาในหน้า `/onsite/people` เมื่อพิมพ์ค้นหาด้วยเลขบัตร 13 หลัก / เบอร์โทร / ชื่อ-นามสกุล หากพบเอกสารที่มีสถานะ `draft` ให้แสดงป้ายกำกับ `[ 🪪 เสียบบัตรแล้ว (รอคัดกรอง) ]` พร้อมปุ่ม Action: `[ ดำเนินการคัดกรองและลงทะเบียน (Step 1) ]`
...
- **FR-STAFF-03 (Step 2 - Personal Info):** Autofill ข้อมูลจาก `card_snapshot` (คำนำหน้า, ชื่อ, นามสกุล, เลข 13 หลัก, เพศ, วันเกิด/อายุ, รูปถ่าย) โดยเจ้าหน้าที่ซักถามเพิ่มเติมเฉพาะ เบอร์โทรศัพท์, ศาสนา, โรคประจำตัว, ประวัติแพ้ยา/อาหาร, และกลุ่มเปราะบาง
