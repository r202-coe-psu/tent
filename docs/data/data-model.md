...
# Smart Shelter — Data Model v3 (CouchDB remote-first)

ระบบหลักเป็น **remote-first**: app เขียนไปที่ **active endpoint** โดยตรง (central ก่อนเสมอ, edge เฉพาะ failover)
และทำ retry/failover ตามสถานะเครือข่าย. ปกติ endpoint คือ CouchDB central; Edge ใช้เฉพาะตอน WAN/central
...
```
                      normal path
device app  ⇄ WAN ⇄  central (CouchDB)
    │                    ⇅
    │ fallback only      │ edge fallback replica / backlog
    └────── LAN ─────⇄ edge server (CouchDB @ศูนย์)
```

- app ใช้ **remote-first write** ไปที่ active endpoint โดยตรง; UI แสดงสถานะส่งข้อมูล/ลองใหม่ตามผลลัพธ์จาก endpoint
- active endpoint ของ device มีได้ **เป้าหมายเดียวต่อเวลา**:
  1. **Central CouchDB** เมื่อ reachable (โหมดปกติ)
  2. **Edge CouchDB บน LAN** เฉพาะเมื่อ WAN/central เข้าไม่ได้
  3. **ไม่มี local-only write queue ในรุ่นนี้**: ถ้าเข้าไม่ได้ทั้ง central/edge ให้ mutation fail พร้อมสถานะ reconnect/retry
...
- ศูนย์ 20,000 คน: traffic ปกติวิ่งกับ **central**; ถ้า WAN/central ล่มระหว่าง onboarding จึง failover ไป
  **LAN กับ edge fallback** — ใช้ retry/backoff ที่ระดับ client และจัด batch ที่ระดับ edge⇄central replicator
...
- central และ edge ออก `_session` cookie แยกกันตาม origin/remote; session ของ edge ใช้ได้แค่
  เรียก edge fallback endpoint เท่านั้น และ **ไม่** grant สิทธิ์เรียก `/api/v1/*` หรือ service API ของ central
- เมื่อ central กลับมา app ต้อง validate/login central session แล้วสลับ active endpoint กลับ central;
  ระหว่างสลับยังคงกติกา active endpoint เดียว
...
- device ไม่เก็บ `_users`; disconnected policy เป็น status-only จึงไม่ใช้ read-only local cache เมื่อไม่มี endpoint ให้เชื่อมต่อ
