...
| --- | --- | --- | --- | --- |
| `shelter_{shelter_code}` | ✓ (ทุกศูนย์) | ✓ (เฉพาะศูนย์ตน) | ✓ (PouchDB) | device ⇄ edge (LAN) · edge ⇄ central (WAN) |
...
| `central_ops` | ✓ เท่านั้น | — | — | — (search_audit, export_job, `counter:shelter`) |
...
- Provisioning script: mint `shelter.code` จาก central counter (`central_ops:counter:shelter` —
  ...
  design docs ที่ central, ติดตั้ง edge server (db เดียวกัน + filtered `_users` replication +
  replication docs edge⇄central) (ดู api-contract §3)
