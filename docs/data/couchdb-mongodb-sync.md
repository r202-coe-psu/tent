...
- worker tail `GET /{db}/_changes?feed=continuous&include_docs=true&since={last_seq}` ของ **ทุก
  `shelter_{code}`** ที่ central (+ `registry` สำหรับ shelter status)
...
{ _id: "evacuee:01H...",            // = CouchDB _id (idempotency)
  ...
  official_code: "SH001-00045" | null,
  updated_at: "..." }
...
{ _id: "SH001:item:01H...", shelter_code, item_name, category, qty_needed, unit, updated_at }
...
{ _id: "donation:01H...", tracking_token, shelter_code, status: "declared|received|expired",
...
{ _id: "SH001", shelter_code, name, status, total_donations_received, occupancy_count, updated_at }
```
...
2. inbound worker poll Mongo `donations` where synced_to_couch=false
   → db.put ลง central shelter_{code}:
...
## 6. Public endpoints map → Mongo (refine api-contract §6)
...
CouchDB central **ไม่มี public endpoint** — §7 ของ api-contract ("สิ่งที่ตั้งใจไม่มี") ยังจริง:
...
- Mongo indexes: `public_persons`: `$text(first_name,last_name_masked)`, `{phone_hash}`,
  `{shelter_code}`; `public_donations`: `{tracking_token}`; `donations`: `{synced_to_couch}`
