...
| **B. Service plane** | staff app เรียกเสริม | REST `/api/v1/*` ที่ **central** (ต้องมี WAN) — เฉพาะงานที่ CouchDB ทำไม่ได้: export, provisioning |
...
| --- | --- | --- | --- |
| `shelter` | `https://edge.local/couch/shelter_{shelter_id}` | ⇄ live+retry | ตลอดที่เห็น edge |
...
POST /api/v1/shelters            { name, capacity, zones[], area_m2?, facilities? }
  → สร้าง shelter:{ulid} ใน registry + shelter_{shelter_id} + _security + design docs ที่ central
...
```
POST /api/v1/exports             { shelter_id, kind: "evacuees|movements|stock|donations",
