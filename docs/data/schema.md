...
> **schema_v 3** — เพิ่ม `asset_photos` สำหรับเก็บรูปภาพทรัพย์สิน. CR-014. (Note: schema_v 2 จาก CR-007 มีการเพิ่ม `country`)
...
| `asset_photos` | [str] | opt | เก็บ URL หรือ Attachment ID ของรูปภาพทรัพย์สิน (CR-014) |
...
> **schema_v 3** — เพิ่ม `house_damage` และ `utilities_status` (CR-014). (schema_v 2: เพิ่ม municipality_zone, community, address - CR-011)
...
| `house_damage` | enum(`100_percent`,`partial`,`minor`,`none`,`unknown`) | opt | ระดับความเสียหายของบ้าน (CR-014) |
| `utilities_status` | [enum(`no_power`,`no_water`,`no_telecom`)] | opt | สถานะสาธารณูปโภคที่ขาดแคลน (CR-014) |

สมาชิก = evacuee ที่ `household_id` ชี้มา (ทางเดียว — ไม่เก็บ list สมาชิกใน household กัน conflict)

**Migration (schema_v 1 → 2):** ลบ `zone` (เดิม = โซนในศูนย์อพยพ ซ้ำซ้อนกับ `evacuee.current_stay.zone`); field ใหม่ทั้งหมด optional → doc เดิมไม่ต้อง backfill; ระบบยังไม่ deploy production ณ วันที่ bump.
...
| `items` | [{`item_id`:str?, `free_text`:str?, `category`:str?, `qty`:num>0, `unit`:str, `condition`:str?, `note`:str?}] | kind=items | `item_id` หรือ `free_text` อย่างใดอย่างหนึ่ง; `item_id` → `item_master:{sku\|ulid}`; `category` = ป้าย label จาก `item_category`; `condition` เช่น "ของใหม่ 100%" (public donor กรอกเอง) |
| `amount_thb` | num>0 | kind=money | — |
| `campaign_id` | str\|null | opt | → `donation_campaign:{ulid}` |
| `logistics` | {`delivery_method`:enum(`self_dropoff`,`parcel`,`shelter_pickup`), `vehicle`:enum(`motorcycle`,`car`,`pickup`,`truck`)?, `slot`:{`date`:str, `from`:str, `to`:str}?, `eta`:ts?, `courier_tracking_no`:str\|null} | opt | **req เมื่อ `channel=public`**; `slot` ชี้ `donation_slot` (§2.13, deterministic ต่อ วัน+เวลา); `vehicle` เฉพาะ self_dropoff/shelter_pickup; `eta` = ต้น slot ที่จอง; `courier_tracking_no` donor เติม/แก้ภายหลังผ่าน ticket (DN-6) |
...
> **schema_v 3** — เพิ่ม `allowed_gender` ใน zones. (schema_v 2: capacity req, status enum - CR-004)
...
| `zones` | [{`code`:str, `name`:str, `capacity`:int>0, `allowed_gender`:enum(`all`,`female_only`,`male_only`)}] | req | `allowed_gender` บังคับใช้ในกฎ GBV Protection (CR-014) |
...
### 3.1.2 `host_house` — `host_house:{ulid}` (New in CR-014)

> เอกสารบ้านพี่เลี้ยง (Host House) สร้างโดยจิตอาสาผ่าน Public Portal สำหรับรองรับผู้อพยพ.

| Field | ชนิด | req | หมายเหตุ |
| --- | --- | --- | --- |
| `host_name` | str | req | ชื่อเจ้าของบ้าน / จิตอาสา |
| `phone` | str | req | เบอร์ติดต่อ |
| `address` | {`full_text`:str, `lat`:num?, `lng`:num?} | req | ที่อยู่ |
| `max_capacity` | int>0 | req | จำนวนคนที่รองรับได้สูงสุด |
| `checklist` | {`has_safe_dry_area`:bool, `has_clean_toilet`:bool, `has_drinking_water`:bool} | req | ประเมินความปลอดภัย |
| `photos` | [str] | opt | URL ภาพถ่ายพื้นที่ |
| `status` | enum(`pending`,`approved`,`rejected`,`closed`) | req | รอการตรวจสอบจากศูนย์ |
...
| `consumption_rate` | num | opt | อัตราการใช้ต่อคน/ครัวเรือน ตาม `distribution_type` |
...
### 4.3 `recipe` — `recipe:{ulid}` · **schema_v 2** (ขยาย field)

| Field | ชนิด | req | หมายเหตุ |
| --- | --- | --- | --- |
| `label` | str | req | ชื่อแสดงผลภาษาไทย เช่น "ข้าวไข่เจียว" |
| `ingredients` | [{`item_master_id`:str, `quantity`:num>0, `uom`:str}] | req | รายการวัตถุดิบและปริมาณ; `item_master_id` → `item_master:{sku\|ulid}` |
| `standard_portions` | num>0 | req | จำนวนที่ผลิตได้ต่อหนึ่งรอบประกอบอาหาร |
| `standard_duration_hours` | num>0 | req | ระยะเวลาปรุงในหน่วยชั่วโมง |
