...
| `official_code` | str\|null | sys | `"{shelter_code}-{seq5}"` เช่น `SH001-00045` — mint service เติม; ห้าม client เขียนเอง |
...
**Index:** `(last_name, first_name)` · `(phone)` · `(official_code)` · `(household_id)` · `(current_stay.status)`
...
| Field | ชนิด | req | หมายเหตุ |
| --- | --- | --- | --- |
| `from_shelter` / `to_shelter` | str | req | shelter code — doc เกิดฝั่งต้นทาง replicate ผ่าน central |
...
## 3. DB `registry` (central-managed → pull ลง edge/device)

### 3.1 `shelter` — `shelter:{code}` (deterministic)
...
## 4. DB `catalog` (central-managed → pull ลง edge/device)
...
### 5.3 `mint_counter` — `mint:{shelter_code}` (deterministic, 1 doc/ศูนย์)

| Field | ชนิด | req | หมายเหตุ |
| --- | --- | --- | --- |
| `last_seq` | int≥0 | req | mint service `_rev`-guard update — แพ้ 409 ให้ retry |
| `minted_total` | int | sys | สถิติ |

---

## 6. Index & view สรุปต่อ db (deploy ใน `_design/app` ตอน provisioning)

| DB | Mango indexes | Views (map/reduce) |
| --- | --- | --- |
| `shelter_*` | evacuee: name, phone, official_code, household_id, stay.status · movement: (evacuee_id, occurred_at) · screening: (evacuee_id, screened_at) · stock_ledger: (item_id, occurred_at) · donation: status, tracking_token_hash, campaign_id · medical: evacuee_id · shift: (date, shift) | `occupancy` (count by stay status) · `stock_balance` (sum qty by item) · `latest_screening` · `meals_served` (sum by date+meal) · `needs_open` |
| `registry` | shelter: status | — |
| `catalog` | item: category · recipe: active | — |
| `central_ops` | export_job: (status, requested_by) · search_audit: occurred_at | — |

## 7. Validation rules (สรุปที่ `validate_doc_update` ต้องบังคับ — ทั้ง central และ edge)
...
5. `shelter_code` ใน doc ต้องตรงกับ db; `official_code` แก้ได้เฉพาะ mint service (role `system_admin`)
