...
| `shelter_id` | str | client | ULID ของ shelter — ใส่ทุก doc ใน `shelter_{shelter_id}` (ใช้ตรวจ doc หลง db) |
| `created_at` / `updated_at` | ts | client | append-only types: สองค่านี้เท่ากันเสมอ |
| `created_by` | str | client | `_users` name ของผู้สร้าง |

---

## 1. DB `shelter_{shelter_id}` — People
...
| `destination` | {`kind`:enum(`home`,`shelter`,`hospital`,`other`), `shelter_id`:str?, `detail`:str?} | opt | ใช้กับ check_out / transfer_out |
| `reason` | str | opt | — |
...
## 2. DB `shelter_{shelter_id}` — Operations
...
| Field | ชนิด | req | หมายเหตุ |
| --- | --- | --- | --- |
| `from_shelter` / `to_shelter` | str | req | shelter_id (ULID) — doc เกิดฝั่งต้นทาง replicate ผ่าน central |
...
5. `shelter_id` ใน doc ต้องตรงกับ db
