...
> **schema_v 2** — เพิ่ม `donor.line_id`/`donor.email` (optional), `items[].category`/`condition`/`note`, `booking_ref`, `tax_receipt_requested`, และ `logistics{}` (วิธีส่ง/ยานพาหนะ/slot/eta/courier tracking) รองรับ public donation + queue booking flow ของหน้า `/donate`. CR-005 §F (DN-2/DN-6/DN-7). field-level canonical ของ [Donation & Queue Booking spec](../features/public-tier-donation-spec.html).
...
| `items` | [{`item_id`:str?, `free_text`:str?, `category`:str?, `qty`:num>0, `unit`:str, `condition`:str?, `note`:str?}] | kind=items | `item_id` หรือ `free_text` อย่างใดอย่างหนึ่ง; `category` = ค่าใน `supply_item.category` enum; `condition` เช่น "ของใหม่ 100%" (public donor กรอกเอง) |
...
| `tax_receipt_requested` | bool | opt | default `false` — donor ขอใบอนุโมทนา/ลดหย่อนภาษี; field PII เพิ่มของใบ **ยังไม่เคาะ (DN-3)** |
