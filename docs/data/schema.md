...
> **schema_v 2** — เพิ่ม `calc_source` (audit trail ของการคำนวณ ingredient จาก SOP ratio). CR-021.
...
**Headcount source — occupancy mapping (CR-022):** derive จาก evacuee ที่ `current_stay.status = 'checked_in'` —
...
→ stock item (`ingredient:rice` → `item:rice`, unit `g`) เป็น `kitchen_requisition` input, `qty_issued`
เริ่ม 0.
...
### 2.7.1 `gas_cylinder_type` — `gas_cylinder_type:{ulid}` · **schema_v 1** (ใหม่)

> **schema_v 1** — reference data สำหรับคำนวณเวลา/ปริมาณการใช้แก๊สหุงต้ม (LPG) ในครัว. CR-021 (ต่อยอด CR-003 T-56). mutable — LWW ผ่าน `touch()`.

| Field | ชนิด | req | หมายเหตุ |
| --- | --- | --- | --- |
| `name` | str | req | เช่น "เตาแรงดันสูง + ถัง 15kg" |
| `capacity_kg` | num>0 | req | น้ำหนักแก๊สเติมต่อถัง (kg) |
| `burn_rate_kg_per_hour` | num>0 | req | อัตราสิ้นเปลือง (kg/ชม.) |
| `time_multiplier` | num>0 | req | ตัวคูณเวลา; default `1` |
