...
> **schema_v 2** — ลบ `zone` เดิม; เพิ่ม `municipality_zone` + `community` (เขต/ชุมชนของที่อยู่เดิม ค่าจาก `master_data`) + เพิ่มฟิลด์ที่อยู่แบบ flat 6 ฟิลด์. CR-011.
...
Design docs / `validate_doc_update` ต้อง deploy ทั้ง central and edge เพราะ edge อาจเป็น active
