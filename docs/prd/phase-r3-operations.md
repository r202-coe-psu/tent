---
title: "Phase R3 PRD: Operations — Donation, Kitchen, Volunteer, SOP & Shelter Reports"
status: active
phase: R3
...
PRD ฉบับนี้กำหนด scope/FR/NFR ของ **Phase R3 (ก.ค.–ส.ค. 2026, Operations Gate 22 ส.ค.)** — phase ที่เปิด module operations หลังบ้านพร้อมกันหลาย track ผู้อ่านคือ PM/SA, PO, ทีมที่เกี่ยวข้อง (K-13 ปิดแล้วตาม Squad Roster) และ QA/Release Owner

R3 ต่อจาก [R2 Foundation](phase-r2-foundation.html): ID numbering เดินต่อ (เริ่ม FR-35, NFR-17, UJ-7, SM-11) และพึ่ง resource backbone (Inventory, reorder threshold, donation intake) ที่ R2 วางไว้ ลำดับ dependency ดู [Roadmap §4](roadmap.html) · [_timeline](../task-breakdown/_timeline.md)

R3 เป็น **Operations Gate (22 ส.ค.)** — phase ที่ scope หนาที่สุดและต้องรันขนาน 5 track; ต้องปิด gate ทันกำหนดเพื่อให้ in-scope ทั้งหมดส่งมอบ **31 ส.ค.** และ go-live ก.ย. เงื่อนไขสำคัญ: **P-02 design (บริษัท) ต้องเสร็จก่อน ก.ค.** มิฉะนั้นทีมรอ design

## 1. Vision

R2 ทำให้ศูนย์เห็นคนเป็นครัวเรือนและเห็น stock R3 ทำให้ศูนย์ "เดินงานประจำวัน" ได้จริง: ของบริจาคไม่ล้นเพราะระบบตัดยอดและ redirect อัตโนมัติ, ครัวกลางวางแผนอาหารจากยอดผู้พักพิงจริง, อาสาสมัครถูกจับคู่กับงานตามทักษะ, ระบบคำนวณ SOP บอกว่าแต่ละวันต้องการวัตถุดิบ/อาสา/ของเท่าไร, และรายงานในศูนย์/การส่งต่อมีระบบรองรับ
...
- **Kitchen Lead** (`kitchen_staff`) ต้องวางแผนอาหารจากยอดผู้พักพิงจริง บันทึกการรับ/ปรุง/แจกจ่ายอาหาร และรู้ว่าต้องเบิกวัตถุดิบจากคลังเท่าไร
- **Shelter Manager** (`shelter_manager`) ต้องรับสมัครอาสา จับคู่ทักษะกับงานที่ขาด และจัดกะ — Volunteer Coordinator ไม่ใช่ role แยก (CR-002)
- **Shelter Manager** ต้องเห็นผลคำนวณ SOP รายวัน (ต้องการวัตถุดิบ/อาสา/ของเท่าไร) เพื่อวางแผนล่วงหน้า
- **Shelter Manager** ต้องเปิดและติดตามรายงานในศูนย์ (ร้องเรียน/ร้องทุกข์ หรือเหตุการณ์ — `kind` grievance|incident) — ไม่มี Security Officer role แยก (CR-002 / CR-040)
- **Shelter Manager** ต้องส่งต่อ (referral) เมื่อศูนย์เต็ม อาหารขาด หรือมีผู้ป่วยฉุกเฉิน

### 2.2 Non-Users (R3)

- EOC/จังหวัด ยังไม่ดึงข้อมูลผ่าน aggregate API (deferred R4 tail ≤14 ก.ย. — FD-14, service แยก)
- ประชาชนค้นหาญาติ (R4 in-scope ส.ค. — family search build ขนานช่วงปลาย R3)
...
- **Shelter Report** — รายงานในศูนย์ (`shelter_report`) แยกประเภทด้วย `kind`: grievance (ร้องเรียน/ร้องทุกข์) | incident (เหตุที่ staff/SM บันทึก); ติดตามจนปิดหรือ escalate → referral — ไม่ใช่ gate check-in/out หรือ occupancy roster ([CR-040](../changes/CR-040-shelter-case-grievance-reframe.md))
...
- referral medical-emergency: SM เป็น referral owner; medical detail อยู่ใน internal shelter scope เท่านั้น และไม่ออก public/API/EOC (ต่อ NFR-5, FD-13)
- สถานะ referral ติดตามได้จนปิด; auditable
- [ASSUMPTION] R3 ทำ referral ภายในเครือข่ายศูนย์ในระบบ; การส่งต่อหน่วยงานภายนอกเต็มรูปเชื่อมผ่าน EOC/Open API (deferred ≤14 ก.ย.)

## 5. Non-Goals (R3)

- ไม่ทำ EOC aggregate API/Open API (deferred ≤14 ก.ย., FD-14 — service แยก) — R3 ผลิตข้อมูล ยังไม่ส่งออก
- ไม่ทำ public family search ใน track นี้ (อยู่ R4 in-scope ส.ค. — ทีม People ทำขนาน)
- ไม่ทำ SOP simulation/what-if scenario (deferred ≤14 ก.ย.) — R3 ทำ calculator จริงเท่านั้น
...
- Shelter Reports: เปิด/ติดตาม `shelter_report` (kind grievance|incident) + escalate atomic → referral ([CR-040](../changes/CR-040-shelter-case-grievance-reframe.md))
- Referral: referral & hand-off ภายในเครือข่ายศูนย์

### 6.2 Out of Scope for R3

- EOC aggregate API + Open API → deferred ≤14 ก.ย. (FD-14)
- Public family search → R4 in-scope ส.ค. (ขนาน)
- SOP simulation/scenario → deferred ≤14 ก.ย.
- External-agency referral integration เต็มรูป → deferred (ผ่าน Open API)
...
5. ~~Security Event ต้องเชื่อม CCTV/access control hardware ไหม~~ — **ปิด (CR-040):** นอก scope · manual intake เท่านั้น

## 10. Assumptions Index

- [ASSUMPTION] FR-41 — การผูก meal กับ Person รายคนเป็น optional; R3 เน้นยอดรวม/ประเภท
- [ASSUMPTION] FR-48 — referral ภายในเครือข่ายศูนย์ใน R3; external-agency เต็มรูปผ่าน Open API (deferred)
