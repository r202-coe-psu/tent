...
status: approved
created: 2026-06-04
updated: 2026-08-14 # §3 FR-34 CR-074 — grant SA / exclusive roles / last-SA / bootstrap admin lock
...
**หมายเหตุ:**
- **FR-34 user creation (แก้ 2026-06-18, CR-074 2026-08-14):** SM สร้าง/ลบได้เฉพาะ **staff** (`registration_staff`/`kitchen_staff`/`warehouse_staff`)
  ใน **ศูนย์ตน** (shelter derive จาก session) — สร้าง `shelter_manager`/`system_admin` หรือข้ามศูนย์ = SA เท่านั้น.
  Grant `system_admin` ได้เฉพาะ caller ที่ถือ app RoleKey `system_admin` (Couch `_admin` mint ผ่าน `/api/v1/users` ไม่ได้).
