...
PRD ฉบับนี้กำหนด scope/FR/NFR ของ **Phase R4** — phase ปิดโครงการส่วน software platform ผู้อ่านคือ PM/SA, PO, Lead pair + ทีมที่เกี่ยวข้อง (K-13 ปิดแล้วตาม Squad Roster) และ QA/Release Owner
...
**EOC architecture (FD-14 + เคาะ 2026-06-11, sync 2026-06-15):** EOC/Open API เป็น **service แยก** จากระบบหลัก — worker/ETL อ่านจาก CouchDB มาสรุปเป็น **MongoDB projection** แล้ว expose เป็น API; **ไม่มี human EOC dashboard / EOC viewer role ในระบบหลัก**. K-17 ปิดแล้วด้วย MongoDB read-model.
...
- **EOC Read-Model** — MongoDB projection ของ service แยก ที่ worker/ETL สรุปจาก CouchDB (K-17 closed)
...
2. **Refresh cadence ของ EOC aggregate read-model:** MongoDB datastore ปิดแล้ว; ยังต้องเคาะ polling/near-real-time interval ใน P-03
