...
# Phase R2 PRD: Foundation — Household, Zoning & Inventory

## 0. Document Purpose

PRD ฉบับนี้กำหนด scope/FR/NFR ของ **Phase R2 (สิงหาคม)** ซึ่งเป็น phase แรกหลัง July MVP ผู้อ่านคือ PM/SA, Product Owner, squad owners (S1-S4) และ QA/Release Owner

R2 ต่อยอดจาก [MVP PRD](../prd-smart-shelter-spec-2026-05-30/prd.html) โดยตรง: FR/NFR/UJ/SM numbering เดินต่อ (เริ่ม FR-21, NFR-12, UJ-5, SM-7) field contract ยังอิง [Data Dictionary](../../data/smart-shelter-data-dictionary.html) (ขยายแบบ additive) ลำดับ phase และ squad ดูใน [Roadmap](roadmap.html)

จุดยืนของ R2: **ปูฐานหลังบ้าน** — ปิด gap ที่ MVP เลื่อนไว้ (Household, zoning) และวาง resource backbone (Inventory) ที่ Donation/Kitchen/SOP ใน R3 ต้องใช้ ยังไม่เปิด module operations เต็ม

## 1. Vision

July MVP พิสูจน์ว่า registration-first flow ระดับ Person ใช้งานจริงได้ แต่หน้างานจริงคนมาเป็น **ครัวเรือน** และศูนย์ต้อง **จัดโซน** ตามกลุ่ม (ครอบครัว, ชาย/หญิง, สัตว์เลี้ยง, เปราะบาง) อีกทั้งของบริจาคเริ่มไหลเข้าทันทีที่เปิดศูนย์ ถ้าไม่มีคลังกลางจะเกิดภาวะ "ของกองล้นศูนย์หนึ่ง ขาดอีกศูนย์" แบบปี 2568
...
- **System Admin** ต้องเพิ่ม role ใหม่และกำหนดสิทธิ์ระดับ field/action โดยไม่กระทบ permission เดิมของ MVP
...
**Description:** ขยาย Person registration ของ MVP ให้ผูกหลาย Person เป็น Household และออก Household Shelter ID/QR เพิ่มจาก Person ID เดิม โดยไม่ทำลาย Person-only flow ที่ MVP ใช้อยู่ Realizes UJ-5
...
**Description:** Shelter Manager จัดสรรผู้พักพิงเข้าโซนตามประเภทและความจุ ต่อยอดจาก capacity warning ของ MVP ที่เดิมเป็นระดับศูนย์ ให้ลงเป็นระดับโซน Realizes UJ-5
...
- Zone capacity เป็น **warning-only** ไม่ hard block (สอดคล้องหลัก capacity ของ MVP) override บันทึก actor
...
**Consequences (testable):**
- Stock Dashboard แสดง on-hand ต่อประเภทต่อศูนย์ และยอดรวมทุกศูนย์ พร้อม last-updated timestamp (ต่อหลัก dashboard MVP)
- ตั้ง reorder threshold ต่อ item ต่อศูนย์ได้; ต่ำกว่าเกณฑ์ → flag "ขาดแคลน"
- ตัวเลขต้อง reconcile กับ Stock Ledger ใน UAT (ต่อ SM แนวเดียวกับ dashboard MVP)
...
**Description:** S1 ขยาย role/permission และ data model แบบ additive รองรับทุก feature R2 และเตรียม R3-R4 โดยไม่ทำลาย MVP

#### FR-34: New Roles & Field-Level Permissions

System Admin กำหนด role ใหม่ (Warehouse/Supply Officer, และ placeholder สำหรับ Kitchen Lead / Volunteer Coordinator) พร้อมสิทธิ์ระดับ field/action
...
- permission เดิมของ MVP role ไม่เปลี่ยนพฤติกรรม (regression test ผ่าน)
- ขยายต่อจาก [Role Permission Matrix](../prd-smart-shelter-spec-2026-05-30/role-permission-matrix.html) เป็น matrix เวอร์ชัน full-system
...
- ไม่ทำ full offline sync สำหรับ inventory/household (offline ยังเน้น registration/screening ตาม MVP)
...
## 7. Cross-Cutting NFRs *(เพิ่มจาก MVP NFR-1..NFR-11)*
...
- **NFR-16** (Compatibility): การขยาย data model ต้อง additive — MVP collections (`evacuees`, `movement_events` ฯลฯ) และ API เดิมต้องทำงานได้โดยไม่ migration ที่ทำลายข้อมูล Validates FR-34
...
1. Donor pre-declaration เปิดให้ public โดยตรง หรือผ่านเจ้าหน้าที่เท่านั้นใน R2? (กระทบ governance + auth surface)
...
- [ASSUMPTION] FR-32 — Donor อาจเป็น public/limited-access role ต้องผ่าน governance review ก่อนเปิด
- [ASSUMPTION] NFR-16 — data-model expansion ทำได้แบบ additive โดยไม่ต้อง destructive migration ของ MVP collections
