...
* **พจนานุกรมคำแปลกลาง:** เพิ่ม `PUBLIC_DONATIONS_I18N` ใน `frontend/src/lib/constants/i18n/public-donations.ts` ครอบคลุมคำศัพท์และประโยคทั้งหมดในกระบวนการบริจาค
* **รองรับ 2 ภาษาในทุก Component ของ Flow บริจาค:**
  * หน้าตารางความต้องการสิ่งของที่ศูนย์เปิดรับ (`PublicDonorNeeds`)
  ...
  * Modal สำหรับแก้ไขรายการสิ่งของ (`EditDonationItemsDialog`) และยกเลิกคำขอรับบริจาค (`CancelDonationDialog`)
* **Formatting Helpers:** ปรับปรุงฟังก์ชันจัดการเวลาและสถานะ (`formatTrackTimestamp`, `formatTrackSchedule`, `deliveryMethodLabel`, `vehicleLabel`) ใน `frontend/src/lib/features/donations/domain/tracking.ts` ให้รับพารามิเตอร์ `lang` ('th' | 'en') เพื่อจัดฟอร์แมตข้อความและวันที่ได้อย่างถูกต้อง

### 1.2 การปรับปรุง Master Data ประเภทศูนย์พักพิงและระบบแผนที่ฟรี (Shelter Type Master & Free OSM Map)
* **Master Data Dynamic Resolution:** ปรับปรุงการแสดงผลประเภทศูนย์พักพิง (`shelter_type`) โดยดึงข้อมูลจาก Master Data API (`/api/public/v1/config/shelter-types`) แทนการ Hardcode
* **Free Map Tilebase (OpenStreetMap):** สลับระบบแผนที่ใน `frontend/src/lib/constants/maps.ts` มาใช้ **OpenStreetMap Raster Tile (`OSM_RASTER_STYLE`)** เป็นค่าเริ่มต้น ซึ่งฟรี 100% ไม่ติด Rate Limit/Error และไม่ต้องใช้ Token
* **Strong TypeScript Definitions:** ขจัด `any` ออกจาก `shelter-map.svelte` โดยประกาศ `MapLibreNamespace` interface และระบุ Type สำหรับ Map, Marker, Popup และ GeoJSON Source อย่างเคร่งครัด

---

## 🔧 2. ฟีเจอร์รองและการปรับปรุงระบบ (Secondary Features & Improvements)

### 2.1 ปรับปรุงหน้าแรก Public Landing Page ตาม [CR-017](changes/CR-017-dashboard-api-architecture.md)
* นำแผง Metrics และ Polling Transparency เดิมที่ถูก Deprecate ออกจาก `frontend/src/routes/(public)/+page.svelte` และ `+page.ts`
* แสดงผลเฉพาะส่วน Hero Metrics ที่เรียบหรู โหลดเร็ว และไม่ยิง Request ไปยัง Endpoint ที่ยกเลิกแล้ว

### 2.2 เติมข้อมูลจำลองศูนย์พักพิง (SH001-SH004) และภูมิลำเนาเดิม [CR-011](changes/CR-011-household-address-fields.md)
* **ข้อมูลศูนย์พักพิงครบถ้วน 100% สำหรับการ Present:** เพิ่มข้อมูลครบ 6 หมวด (Hero, อาคาร/โซน, สุขอนามัย, สาธารณูปโภค, การเดินทาง/ความสูง, ข้อมูลผู้จัดการศูนย์ และ FAQ) ใน `frontend/scripts/seed.ts` และ `backend/apiapp/modules/shelter/use_case.py`
...
### 2.3 การผนวกรวม Typography Tokens ล่าสุดจาก Develop
* Merge การเปลี่ยนแปลงล่าสุดจาก `origin/develop` โดยปรับขนาดตัวอักษรเป็น Rem-based tokens (`text-2xs`, `text-xs`, `0.625rem`) ตามดีไซน์ซิสเต็มใหม่ พร้อมรักษาฟังก์ชันและการแปลภาษาเดิมไว้ครบถ้วน
